...
사용자가 신규 기능·특정 component·특정 화면 영역처럼 **범위를 명시해 CT35 평가를 요청한 경우**, 프로젝트 전체 CT35를 다시 채점하지 않고 해당 기능과 그 기능이 직접 사용하는 공통 contract만 평가한다. 35개 항목과 각 항목의 고정 배점은 변경하지 않는다.

먼저 1~35 각 항목을 다음 세 범위로 분류한다.

- **적용:** 해당 기능이 그 항목의 token, component, responsive, state 또는 책임을 직접 구현·소유한다. 해당 기능 소스와 직접 연결된 shared owner까지 평가한다.
- **간접 적용:** 해당 기능이 기존 공통 token/helper/component/modal/responsive owner를 재사용한다. 공통 primitive 전체를 재평가하지 않고, 해당 기능이 canonical contract를 올바르게 재사용하는지와 기능 전용 override가 contract를 우회하는지만 평가한다.
- **N/A:** 해당 기능과 직접 관계가 없고 shared contract 변경도 없는 항목이다. 구현을 억지로 요구하지 않으며 감점과 집중평가 점수 계산에서 제외한다.

집중평가에서는 다음 경계를 지킨다.

- 기능과 무관하게 기존 Main/Add에 이미 존재하던 결함은 해당 기능 점수에 전가하지 않는다.
- 기존 공통 결함이 실제로 해당 기능에도 도달해 사용자 결과나 유지보수 contract를 깨뜨리는 경우에만 근거를 명시해 해당 항목에 반영한다.
- 신규 기능 때문에 shared token/helper/component/modal/responsive owner가 수정됐다면, 그 **변경된 shared contract의 직접 회귀 범위**까지 확인한다. 단, 이를 이유로 프로젝트 전체 CT35를 다시 평가하지 않는다.
- canonical primitive를 재사용했더라도 기능 전용 selector·literal·helper가 같은 semantic을 중복 소유하거나 공통 contract를 우회하면 해당 항목에서 감점한다.
- `N/A` 항목을 점수 확보 목적으로 억지로 적용 대상으로 만들거나 불필요한 공통화를 요구하지 않는다.
- 집중평가 결과는 Main/Add 전체 CT35 점수를 대체하지 않는 **별도 범위 평가**로 기록한다.
- 집중평가가 Main과 Add 양쪽에 걸치면 기존 원칙대로 Main/Add를 각각 독립 채점한다. 한쪽에만 속한 기능은 해당 영역만 채점하고 반대 영역은 `N/A`로 둔다.
- 집중평가의 단일 요약 점수가 필요하면 `적용`과 `간접 적용`으로 채점된 항목만 번호당 한 번 단순 평균하고 `N/A`는 분모에서 제외한다. 카테고리별 요약도 동일하게 해당 카테고리의 적용 항목만 사용한다.
...
| 항목 | 적용 범위 | 점수 | 판정 | 핵심 근거 |
|---|---|---:|---|---|
| 1. 색상·Semantic Color | 적용/간접 적용/N/A | 00/100 또는 N/A | PASS/MINOR/... | 해당 기능과 직접 연결된 token/selector 근거 |

전체 평가에서는 1~35를 모두 기록하고, 특정 번호 요청에서는 해당 항목만 평가한다. 특정 기능 요청에서는 먼저 1~35의 적용 범위를 분류한 뒤 `적용`·`간접 적용` 항목만 채점한다. 카테고리 평균은 문서 마지막 분류표를 사용하되 Main/Add를 합산하지 않는다.
...
### 평가 기준
- 메인 card/tooltip primitive를 재사용하는가
...
- 환경과 무관하게 실제 endpoint 응답이 확인되기 전 Market AI UI를 mount하지 않고, 전체 연결 실패 시 signal panel을 제거한 채 `OFFLINE`으로 종료하는가
...
이 문서는 **35개 평가 항목, 고정 배점, 항목별 장기 판정 기준 또는 평가 범위 적용 규칙이 바뀔 때만 수정**한다. 단순 CSS/JS 수정, 특정 버그·viewport 조정, QA PASS, 특정 수치 변화, 과거 작업 이력은 누적하지 않는다. 전역 평가 방법론과 A/B/C·100점·종료 기준은 `dashboard_evaluation_guide.md`에서만 관리한다.
