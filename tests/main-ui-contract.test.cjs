...
test('월간 손익 캘린더는 기존 계산·modal·날짜 이동 contract를 재사용한다',()=>{
  ...
  assert.match(special,/\.monthly-calendar-separate-profit\.separate-profit-toggle\{[^}]*--dashboard-control-height:29px;[^}]*--dashboard-control-font-size:10\.5px;[^}]*--dashboard-control-state-font-size:10px;[^}]*--dashboard-control-state-min-width:28px;[^}]*--dashboard-control-pad-x:8px;[^}]*gap:var\(--space-md\);[^}]*padding:0 var\(--dashboard-control-pad-x\)/,'Phone 월간 별도수익 toggle은 위치만 Phone 배치를 따르고 Web/Tablet control 크기와 간격을 그대로 사용해야 한다');
