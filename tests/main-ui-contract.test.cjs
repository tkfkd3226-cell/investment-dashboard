...
test('Topbar action 라벨은 Web full/short와 Tablet 축약명·icon 조합을 사용한다',()=>{
  ...
  assert.match(special,/@media \(min-width:1101px\) and \(max-width:1439px\)\{[^]*?\.date-picker\{grid-template-columns:minmax\(0,1fr\) auto\}[^]*?\.date-picker-center\{[^}]*width:100%[^}]*max-width:calc\(var\(--topbar-date-select-width\) \+ var\(--topbar-date-select-width\) \+ var\(--topbar-layout-gap\)\)[^}]*\}[^]*?\.month-select,[^]*?\.day-select\{[^}]*min-width:0[^}]*max-width:var\(--topbar-date-select-width\)[^}]*flex:1 1 var\(--topbar-date-select-width\)/,'1101~1439px Web은 full/short label과 무관하게 두 날짜 selector가 action 영역을 침범하기 전에 가로 수축해야 한다');
