...
test('월간 손익 캘린더는 기존 계산·modal·날짜 이동 contract를 재사용한다',()=>{
  ...
  assert.match(common,/--monthly-separate-profit-height:29px;[^]*?--monthly-separate-profit-font-size:10\.5px;[^]*?--monthly-separate-profit-state-font-size:10px;[^]*?--monthly-separate-profit-state-min-width:28px;[^]*?--monthly-separate-profit-pad-x:8px;[^]*?--monthly-separate-profit-state-height:18px;/,'월간 별도수익 toggle skin은 common token 한 곳에서만 정의해야 한다');
  assert.match(common,/\.monthly-calendar-separate-profit\.separate-profit-toggle\{[^}]*height:var\(--monthly-separate-profit-height\);[^}]*min-height:var\(--monthly-separate-profit-height\);[^}]*gap:var\(--monthly-separate-profit-gap\);[^}]*padding:0 var\(--monthly-separate-profit-pad-x\)/,'월간 별도수익 outer line 높이·간격·padding은 공통 token만 사용해야 한다');
  assert.match(common,/\.monthly-calendar-separate-profit\.separate-profit-toggle strong\{[^}]*min-width:var\(--monthly-separate-profit-state-min-width\);[^}]*height:var\(--monthly-separate-profit-state-height\);[^}]*padding:0 var\(--monthly-separate-profit-state-pad-x\)/,'월간 ON/OFF badge 크기도 공통 token만 사용해야 한다');
  assert.match(common,/\.monthly-calendar-separate-profit \.separate-profit-toggle-label\{display:inline\}/,'월간 별도수익 라벨은 모든 viewport에서 같은 skin 계약을 유지해야 한다');
  assert.match(common,/\.monthly-calendar-separate-profit\.separate-profit-toggle strong \.control-text-optical\{transform:none\}/,'월간 ON/OFF optical 정렬도 모든 viewport에서 동일해야 한다');
  assert.doesNotMatch(special,/\.monthly-calendar-separate-profit\.separate-profit-toggle\{[^}]*--dashboard-control-|\.monthly-calendar-separate-profit\.separate-profit-toggle strong\{/,'Phone CSS는 월간 별도수익 skin을 다시 정의하면 안 되고 위치만 소유해야 한다');
