...
test('Topbar action 라벨은 Web full/short와 Tablet 축약명·icon 조합을 사용한다',()=>{
  ...
  assert.match(common,/\.date-picker\{[^}]*grid-template-columns:minmax\(0,calc\(var\(--topbar-date-select-width\) \+ var\(--topbar-date-select-width\) \+ var\(--topbar-layout-gap\)\)\) minmax\(0,1fr\) auto/,'Web 날짜 영역은 임의 viewport breakpoint 없이 실제 남는 폭에 따라 수축 가능한 grid track을 사용해야 한다');
