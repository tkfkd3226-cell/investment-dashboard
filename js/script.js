...
const securitySymbolSwatch=name=>(!ACTIVE_DATE||securityChartNamesForDate(ACTIVE_DATE).includes(name))?assetColorSwatch(securityAllocationColor(name)):'';
  ...
  const groups=[
    ...
    },
    {
      ...
      items:[
        ...
        ...(PERSONAL_VIEW_UNLOCKED?[{type:'link',url:'add/calc.html',icon:'calculator',title:'투자 계산기'}]:[])
      ]
    },
    {
      label:'전체',
  ...
  document.getElementById('tabs').innerHTML=`
    <div class="date-picker">
      ...
      <div class="date-picker-action">
        ...
        ${PERSONAL_VIEW_UNLOCKED?`<a class="date-tool-btn date-tool-btn-desktop topbar-calc-action" href="add/calc.html" target="_blank" rel="noopener noreferrer" title="투자 계산기" aria-label="투자 계산기" style="text-decoration:none">
        ...
        <div class="date-action-menu-wrap">
          ...
          <div id="dateActionMenu" class="date-action-menu mobile-combined-menu" aria-label="화면 목차"><div class="mobile-nav-head"><div class="mobile-nav-head-title"><span>목차</span></div><label class="mobile-date-pin-control" for="mobileDatePinToggle"><span>날짜 선택 고정</span><input type="checkbox" id="mobileDatePinToggle" role="switch" ${mobileDatePinned()?'checked':''} onchange="setMobileDatePinned(this.checked)"><span class="mobile-date-pin-track" aria-hidden="true"><span></span></span></label><button type="button" onclick="closeDateActionMenu()" aria-label="목차 닫기">×</button></div>${renderUnifiedMobileMenuContent()}</div>
        </div>
      </div>
    </div>`;
  syncMobileTopbarState();
...
function mobileViewToggle(key){
  ...
  return `<button type="button" class="section-control-chip section-action-chip mobile-view-toggle" data-mobile-view-button="${key}" onclick="toggleMobileDataView('${key}')">${label}</button>`;
}
...
function renderCharts(x){
  ...
  return `<section id="investment-analysis"><div class="section-title"><h2><span class="section-title-icon">🗓️</span>투자 기간 분석</h2><p class="section-control-chip section-basis-chip">삼성증권1 기준</p></div><div class="grid chart-grid">
...
function renderPensionCharts(x){
  ...
  return `<section id="pension-investment-analysis" class="pension-chart-block"><div class="section-title"><h2><span class="section-title-icon">🗓️</span>투자 기간 분석</h2><p class="section-control-chip section-basis-chip">퇴직연금 기준</p></div><div class="grid chart-grid">
