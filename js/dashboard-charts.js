...
function renderCharts(x,separateProfitHtml=''){
  ...
  <div class="chart-card" id="chart-symbol"><div class="chart-head"><div><h3><span class="section-title-icon chart-icon" data-section-title-icon="barChart" aria-hidden="true"></span>종목별 누적손익</h3></div><div class="chart-head-actions">${symbolChartToggle('securities')}${chartWebExpandButton()}</div></div>${chartScrollButton()}<div class="chart-wrap"><svg class="chart" id="chartSymbol"></svg></div><div class="chart-legend" id="securitiesSymbolLegend">${chartLegendHtml('securitiesSymbol')}</div><div class="chart-note symbol-summary-grid">${orderedSymbols.map(h=>symbolCard(h,symbolTotal)).join('')}</div><div class="section-explainer symbol-summary-note">${hasSymbolBuyFlow?'<strong class="summary-note-key">ㆍ기여도:</strong> 누적손익 합계 기준&nbsp;&nbsp;<strong class="summary-note-key">ㆍ수익률:</strong> 누적손익 ÷ 투자원금&nbsp;&nbsp;<strong class="summary-note-key">ㆍ전일대비 변동률:</strong> 전일대비 변동액 ÷ (전일 평가액 + 당일 매수금액)':'<strong class="summary-note-key">ㆍ기여도:</strong> 누적손익 합계 기준&nbsp;&nbsp;<strong class="summary-note-key">ㆍ수익률:</strong> 누적손익 ÷ 투자원금&nbsp;&nbsp;<strong class="summary-note-key">ㆍ전일대비 변동률:</strong> 전일대비 변동액 ÷ 전일의 평가금액'}</div></div>
