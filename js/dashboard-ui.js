...
function renderHoldings(x){
  ...
  return `<section id="securities-holdings" ${mobileViewAttrs('holdings')}><div class="section-title"><h2><span class="section-title-icon" data-section-title-icon="folder" aria-hidden="true"></span>증권계좌 보유분</h2>${mobileViewToggle('holdings')}</div><div id="holdings-table-view" class="mobile-scroll table-view"><table class="dashboard-data-table hold-position-table"><caption class="visually-hidden">증권계좌 보유분</caption><thead><tr><th scope="col">종목명</th><th scope="col">수량</th><th scope="col">평단</th><th scope="col">투자원금</th><th scope="col">현재가</th><th scope="col">평가금액</th><th scope="col">평가손익</th><th scope="col" class="table-cell-center">손익률</th></tr></thead><tbody>${orderedHoldings.map(h=>`<tr class="hold-row"><th scope="row"><span class="holding-name-text">${h.name}</span>${securitySymbolSwatch(h.name)}</th><td class="num">${fmt(h.qty)}</td><td class="num">${fmt(h.avgPrice ?? (h.qty?h.cost/h.qty:0))}</td><td class="num">${fmt(h.cost)}</td><td class="num">${fmt(h.price)}</td><td class="num">${fmt(h.evalAmount)}</td><td class="num ${tableCls(h.profit)}">${fmt(h.profit)}</td><td class="num table-cell-center ${tableCls(h.returnRate)}">${pct(h.returnRate)}</td></tr>`).join('')}<tr class="summary-row"><th scope="row">보유종목 합계</th><td class="num">-</td><td class="num">-</td><td class="num">${fmt(holdCost)}</td><td class="num">-</td><td class="num">${fmt(holdEval)}</td><td class="num ${tableCls(holdProfit)}">${fmt(holdProfit)}</td><td class="num table-cell-center ${tableCls(holdReturn)}">${pct(holdReturn)}</td></tr><tr><th scope="row">증권계좌 현금</th><td class="num">-</td><td class="num">-</td><td class="num">${fmt(cash)}</td><td class="num">-</td><td class="num">${fmt(cash)}</td><td class="num">0</td><td class="num table-cell-center">0.00%</td></tr><tr class="summary-row"><th scope="row">총계(보유분+현금)</th><td class="num">-</td><td class="num">-</td><td class="num">${fmt(totalCostWithCash)}</td><td class="num">-</td><td class="num">${fmt(totalEvalWithCash)}</td><td class="num ${tableCls(holdProfit)}">${fmt(holdProfit)}</td><td class="num table-cell-center ${tableCls(totalReturnWithCash)}">${pct(totalReturnWithCash)}</td></tr></tbody></table></div><div id="holdings-card-view" class="mobile-card-view">${cards}</div></section>`;
}
...
function renderAccounts(x){
  ...
  const totalRow=`<tr class="summary-row"><th scope="row" class="accounts-name">합계</th><td class="num">${fmt(v.totalPrincipal)}</td><td class="num ${tableCls(v.totalProfit)}">${fmt(v.totalProfit)}</td><td class="num table-cell-center ${tableCls(v.totalReturn)}">${pct(v.totalReturn)}</td><td class="accounts-memo"><span class="accounts-memo-text">${totalMemo}</span>${accountMemoInfoButton(totalMemo)}</td></tr>`;
  return `<section id="accounts-summary" ${mobileViewAttrs('accounts')}><div class="section-title"><h2><span class="section-title-icon" data-section-title-icon="list" aria-hidden="true"></span>계좌별 성과 요약</h2><div class="section-title-actions">${separateProfitControl(x,'section-inline')}${mobileViewToggle('accounts')}</div></div><div id="accounts-table-view" class="mobile-scroll accounts-scroll table-view"><table class="dashboard-data-table accounts-table"><caption class="visually-hidden">계좌별 성과 요약</caption><thead><tr><th scope="col" class="accounts-name-head">구분</th><th scope="col">투자원금</th><th scope="col">누적손익</th><th scope="col" class="table-cell-center">수익률</th><th scope="col" class="table-cell-text">메모</th></tr></thead><tbody>${rows.map(r=>`<tr><th scope="row" class="accounts-name">${r[0]}</th><td class="num">${r[1]?fmt(r[1]):'-'}</td><td class="num ${tableCls(r[2])}">${fmt(r[2])}</td><td class="num table-cell-center ${r[1]?tableCls(r[3]):''}">${r[1]?pct(r[3]):'-'}</td><td class="accounts-memo"><span class="accounts-memo-text">${accountMemoTableHtml(r[4])}</span>${accountMemoInfoButton(r[4])}</td></tr>`).join('')}${totalRow}</tbody></table></div><div id="accounts-card-view" class="mobile-card-view">${cards}</div>${hiddenNote}</section>`;
}
