  ...
  document.getElementById('tabs').innerHTML=`
    <div class="date-picker">
      ...
      <div class="date-picker-action" role="group" aria-label="대시보드 도구">
        ...
        <button type="button" class="date-tool-btn topbar-monthly-action" title="월간 손익" aria-label="월간 손익" data-dashboard-action="open-monthly-calendar">
        ...
        <div class="date-action-menu-wrap">
          ...
          <div id="dateActionMenu" class="date-action-menu mobile-combined-menu" aria-label="화면 목차">
            <div class="responsive-nav-menu-layout"><div class="mobile-nav-head"><label class="mobile-date-pin-control" for="mobileDatePinToggle"><span>날짜 선택 고정</span><input type="checkbox" class="control-switch-input" id="mobileDatePinToggle" role="switch" ${mobileDatePinned()?'checked':''} data-dashboard-change="mobile-date-pin"><span class="control-switch-track" aria-hidden="true"><span class="control-switch-thumb"></span></span></label><div class="mobile-nav-head-actions"><button type="button" class="control-icon-button-compact mobile-nav-theme-action" data-theme-toggle title="${currentTheme()==='dark'?'밝은 모드로 전환':'다크 모드로 전환'}" aria-label="${currentTheme()==='dark'?'밝은 모드로 전환':'다크 모드로 전환'}" data-dashboard-action="toggle-theme"><span data-theme-toggle-icon>${themeToggleIconMarkup(currentTheme()==='dark')}</span></button><button type="button" class="control-icon-button-compact mobile-nav-corner-action" data-corner-theme-toggle title="${currentCornerTheme()==='rounded'?'각진 모서리로 전환':'둥근 모서리로 전환'}" aria-label="${currentCornerTheme()==='rounded'?'각진 모서리로 전환':'둥근 모서리로 전환'}" data-dashboard-action="toggle-corner-theme"><span data-corner-theme-toggle-icon>${cornerThemeToggleIconMarkup(currentCornerTheme()==='rounded')}</span></button><button type="button" class="control-icon-button-compact" data-dashboard-action="close-date-menu" aria-label="목차 닫기">${navIconSvg('close')}</button></div></div>${renderResponsiveNavigationMenuContent()}</div>
          </div>
        </div>
      </div>
    </div>`;
...
function syncRealtimeQuotesModalGeometry(){
  ...
  const fallbackHeight=parseFloat(style.getPropertyValue('--realtime-monitor-fallback-height'))||availableHeight;
  const width=Math.min(naturalWidth,availableWidth);
  const initialHeight=Math.min(fallbackHeight,availableHeight*.86);
  const height=Math.min(realtimeMonitorContentHeight>0?realtimeMonitorContentHeight+2:initialHeight,availableHeight);
...
function clearRealtimeQuotesPreparation(){
  if(realtimeMonitorPrepareTimer){window.clearTimeout(realtimeMonitorPrepareTimer);realtimeMonitorPrepareTimer=0}
  document.getElementById('realtimeQuotesModal')?.classList.remove('realtime-quote-preparing');
}
function revealPreparedRealtimeQuotesModal(){
  const modal=document.getElementById('realtimeQuotesModal');
  if(!modal||!modal.classList.contains('realtime-quote-preparing')||!marketAiMonitorAvailable)return;
  const returnFocus=realtimeMonitorPendingReturnFocus;
...
function openRealtimeQuotesModal(returnFocus=null){
  ...
  if(phoneUi()||realtimeMonitorContentHeight>0){
    clearRealtimeQuotesPreparation();
    realtimeMonitorPendingReturnFocus=null;
    if(needsLoad)frame.setAttribute('src',MARKET_AI_MONITOR_URL);
    const closeButton=modal.querySelector('.realtime-quote-close');
    openDashboardModal(modal,{initialFocus:closeButton,returnFocus,fallbackSelector:'[data-dashboard-action="open-realtime-quotes"]:not([hidden])'});
    syncRealtimeQuotesModalGeometry();
    return;
  ...
  realtimeMonitorPendingReturnFocus=returnFocus;
  clearRealtimeQuotesPreparation();
  modal.classList.add('realtime-quote-preparing');
  syncRealtimeQuotesModalGeometry();
  realtimeMonitorPrepareTimer=window.setTimeout(revealPreparedRealtimeQuotesModal,REALTIME_MONITOR_PREPARE_TIMEOUT_MS);
  if(needsLoad)frame.setAttribute('src',MARKET_AI_MONITOR_URL);
}
...
function ensureKrxActionModal(){
  ...
  modal.innerHTML=`<div class="action-modal-card krx-action-card" role="dialog" aria-modal="true" aria-labelledby="krxActionTitle">
    ...
    <p id="krxActionDescription" class="action-modal-description">최신/누락 반영은 오늘 데이터와 누락 거래일을 생성·보완합니다. 재갱신은 선택일을 다시 조회하며, 오늘은 15:30 전 장중 가격, 15:30부터 정규장 종가를 반영합니다. 과거 거래일은 정규장 종가를 반영합니다.</p>
...
function renderSecuritiesPerformanceSummary(x){
  ...
  return `<div class="securities-subsection securities-summary-block asset-overview"><div class="section-title"><h3><span class="section-title-icon" data-section-title-icon="chart" aria-hidden="true"></span>성과 요약</h3><div class="chart-head-actions">${separateProfitControl(x,'section-inline')}<span data-accounts-view-toggle-wrap${accountsMode?'':' hidden'}>${mobileViewToggle('accounts')}</span>${securitiesPerformanceViewSwitch()}</div></div>${renderSecuritiesSummaryCards(x,{hidden:accountsMode})}${renderAccounts(x,{hidden:!accountsMode})}</div>`;
}
function renderSecuritiesChartsBlock(x){
  return sectionToSecuritiesBlock(renderCharts(x,separateProfitControl(x,'chart-inline')),'charts-block');
}
function renderSecuritiesLedgerBlock(x){
  return sectionToSecuritiesBlock(renderResultSummary(x),'ledger-block');
}
function renderSecuritiesSourceBlock(x){
  return isLedgerCheckDate(x.date)?sectionToSecuritiesBlock(renderSourceTables(x),'source-block'):'';
}
function renderSecuritiesSection(x){
  return `<section id="securities-section"><div class="section-title"><h2><span class="section-title-icon" data-section-title-icon="bank" aria-hidden="true"></span>증권계좌 현황</h2></div><div class="securities-band">${renderSecuritiesPerformanceSummary(x)}${renderSecuritiesAssetDetail(x)}${renderSecuritiesChartsBlock(x)}${renderSecuritiesLedgerBlock(x)}${renderSecuritiesSourceBlock(x)}</div></section>`;
}


function renderResultSummary(x){
  const c=dataState.portfolio.constants,v=separateProfitView(x);
  const outsideCash=outsideCashForDate(x.date),outsideCashSnapshot=outsideCashSnapshotForDate(x.date);
  ...
  const outsideCashFlowText=outsideCashSnapshot&&Number(outsideCashSnapshot.amount)===outsideCash
    ? `${outsideCashSnapshot.date} 확인값 ${won(outsideCash)}`
    : `${outsideCashSnapshot?`${outsideCashSnapshot.date} 확인값 ${won(outsideCashSnapshot.amount)}`:'추적 현금 기준값'} → 현재 ${won(outsideCash)}`;
...
function renderHoldings(x){
  ...
  const rows=orderedHoldings.map(h=>{
    ...
    cells:[
      ...
      {className:`num ${cls(h.profit)}`,html:fmt(h.profit)},
  ...
  const summaryRows=['holdings','cash','total'].map(id=>{
    ...
    return {
      ...
      cells:[
        ...
        {className:`num ${cls(row.profit)}`,html:fmt(row.profit)},
        {className:`num table-cell-center ${cls(row.returnRate)}`,html:pct(row.returnRate)},
...
function securitySaleMarkerHtml(row,{interactive=true}={}){
  ...
  const title=`<span class="security-sale-marker-name">${escapeHtml(String(row?.name||''))}</span>`;
  if(!interactive)return title;
  return `<span class="security-sale-marker" ${securitySaleTooltipAttrs(row)}>${title}</span>`;
}
...
function renderSecuritiesChangeBlock(x){
  ...
  const rows=orderedRows.map(r=>{
    ...
    labelClass:`asset-change-asset-col${saleTooltipAttrs?' security-sale-cell':''}`,
    labelAttrs:saleTooltipAttrs,
    labelHtml:`${securitySaleMarkerHtml(r,{interactive:false})}${securitySymbolSwatch(r.name)}`,
    cells:[
  ...
  const summaryRows=[{
    ...
    cells:[
      ...
      {className:'num asset-change-delta-col',html:renderAssetDayChangeValue({
        ...
        amountClass:cls(change.dayChange),
        rateClass:cls(change.dayRate)
      })}
    ]
  }];
...
function renderCombined(x){
  ...
  const tableBody=`<tr><th scope="row">퇴직연금</th><td class="num">${fmt(x.pensionEval)}</td><td class="num">${fmt(x.pensionPrincipal)}</td><td class="num combined-profit-col ${cls(x.pensionProfit)}">${fmt(x.pensionProfit)}<span class="combined-mobile-return ${cls(x.pensionReturn)}"> (${mobileReturnPct(x.pensionReturn)})</span></td><td class="num table-cell-center combined-return-col ${cls(x.pensionReturn)}">${pct(x.pensionReturn)}</td></tr><tr><th scope="row">증권계좌</th><td class="num">${fmt(v.totalResult)}</td><td class="num">${fmt(v.totalPrincipal)}</td><td class="num combined-profit-col ${cls(v.totalProfit)}">${fmt(v.totalProfit)}<span class="combined-mobile-return ${cls(v.totalReturn)}"> (${mobileReturnPct(v.totalReturn)})</span></td><td class="num table-cell-center combined-return-col ${cls(v.totalReturn)}">${pct(v.totalReturn)}</td></tr><tr class="summary-row"><th scope="row">합산</th><td class="num">${fmt(v.combinedResult)}</td><td class="num">${fmt(v.combinedPrincipal)}</td><td class="num combined-profit-col ${cls(v.combinedProfit)}">${fmt(v.combinedProfit)}<span class="combined-mobile-return ${cls(v.combinedReturn)}"> (${mobileReturnPct(v.combinedReturn)})</span></td><td class="num table-cell-center combined-return-col ${cls(v.combinedReturn)}">${pct(v.combinedReturn)}</td></tr>`;
...
function accountMemoTableHtml(text,{joinFirstTwo=false,highlightSourceLink=true}={}){
  ...
  const sourceLinkPattern=/([+-]?[\d,]+원은 계좌1 투자원금 검산의 레버수익 재투입·VIP 수익 재투입)/;
...
function renderAccounts(x,{hidden=false}={}){
  ...
  const account1PrincipalAdjustmentMemo=account1PrincipalAdjustment
    ? (Math.abs(account1PrincipalAdjustment+account1DerivedAdjustmentBasis)<0.5
      ? ` 투입원금 조정 ${signed(account1PrincipalAdjustment)}원은 계좌1 투자원금 검산의 레버수익 재투입·VIP 수익 재투입을 전체 투입원금 기준으로 조정한 값.`
  ...
  const totalRow=`<tr class="summary-row"><th scope="row">합계</th><td class="num">${fmt(totalResult)}</td><td class="num">${fmt(totalPrincipal)}</td><td class="num performance-profit-col ${cls(v.totalProfit)}">${fmt(v.totalProfit)}<span class="performance-inline-return ${cls(totalReturn)}"> (${mobileReturnPct(totalReturn)})</span></td><td class="num table-cell-center performance-return-col ${cls(totalReturn)}">${pct(totalReturn)}</td><td class="table-cell-text accounts-memo data-table-sub"><span class="accounts-memo-text">${accountMemoTableHtml(totalMemo)}</span>${accountMemoInfoButton(totalMemo)}</td></tr>`;
...
function renderSourceTables(x){
  const c=dataState.portfolio.constants,
    ...
    internalCashPrincipalNet=securityInternalCashPrincipalNetForDate(x.date),
    ...
    internalCashNetRow=internalCashPrincipalNet?sourceTableRow('내부 현금 순이동',internalCashPrincipalNet,{kind:'derived'}):'',
  ...
  const trackedRows=`${sourceTableRow('금 판매액 투입',account1GoldSource,{kind:'base'})}${sourceTableRow('VIP 금 투입분',account2GoldSource,{kind:'base'})}${sourceTableRow('근로소득 투입액',c.laborNetPrincipal,{kind:'base'})}${extraRow}${excludedRow}${sourceTableRow('레버수익 재투입',c.tossReinvestedToAccount1,{kind:'derived'})}${sourceTableRow('VIP 재투입-금',vipReinvestLessGold,{kind:'derived'})}${sourceTableRow('임시자금 투입',temporarySource,{kind:'derived'})}${sourceTableRow('원금 회수',principalRecovery,{kind:'derived'})}${internalCashNetRow}${reconciliationRow}${sourceTableRow('합계',performancePrincipal,{summary:true})}`;
