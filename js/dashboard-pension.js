...
function pensionRow(r,total){const w=total?r.evalAmount/total*100:0,safeW=Math.max(0,Math.min(100,w)),weight=safeW.toFixed(1),name=String(r.name||'');return `<tr><td><strong>${mobileTableProductName(r.name)}</strong>${pensionProductSwatch(r.name)}</td><td class="num table-cell-center">${fmt(r.qty)}</td><td class="num">${fmt(r.qty?r.cost/r.qty:0)}</td><td class="num">${fmt(r.cost)}</td><td class="num">${fmt(r.evalAmount)}</td><td class="num ${tableCls(r.profit)}">${fmt(r.profit)}</td><td class="num table-cell-center ${tableCls(r.returnRate)}">${pct(r.returnRate)}</td><td><div class="bar-box" role="progressbar" aria-label="${escapeHtml(name)} 비중" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${weight}" aria-valuetext="${weight}%"><div class="bar-fill" aria-hidden="true" style="width:${weight}%;background:${pensionSeriesColor(r.name)}"></div></div><div class="small">${weight}%</div></td></tr>`}
function pensionCashRow(cash,total,cost=39408){const w=total?cash/total*100:0,safeW=Math.max(0,Math.min(100,w)),weight=safeW.toFixed(1),profit=cash-cost,ret=cost?profit/cost*100:0;return `<tr><td><strong>현금성자산</strong></td><td class="num table-cell-center">1</td><td class="num">${fmt(cost)}</td><td class="num">${fmt(cost)}</td><td class="num">${fmt(cash)}</td><td class="num ${tableCls(profit)}">${fmt(profit)}</td><td class="num table-cell-center ${tableCls(ret)}">${pct(ret)}</td><td><div class="bar-box" role="progressbar" aria-label="현금성자산 비중" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${weight}" aria-valuetext="${weight}%"><div class="bar-fill bar-gray" aria-hidden="true" style="width:${weight}%"></div></div><div class="small">${weight}%</div></td></tr>`}
