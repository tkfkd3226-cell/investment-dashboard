...
function pensionSectionInputs(x){
  ...
  return {pensionCashCost,pensionHeldCost,pensionHeldProfit,pensionHeldReturn,orderedPensionRows};
}
function renderPensionOverview(x){
  return `<div class="asset-overview pension-overview"><div class="section-title"><h3><span class="section-title-icon" data-section-title-icon="chart" aria-hidden="true"></span>성과 요약</h3></div><div class="grid metric-grid asset-summary-metric-grid">${metricCard('평가금액',won(x.pensionEval),pensionEvaluationBasisText(x.date),true,'',pensionEvaluationMobileSubText(x))}${metricCard('납입원금',won(x.pensionPrincipal),'최근 적립금 반영',false,'','최근 적립금 반영')}${metricCard('운용손익',won(x.pensionProfit),'평가금액 - 납입원금',false,cls(x.pensionProfit))}${metricCard('운용수익률',pct(x.pensionReturn),'운용손익 ÷ 납입원금',false,cls(x.pensionReturn))}</div></div>`;
}
function renderPensionAssetDetail(x){
  const {pensionCashCost,pensionHeldCost,pensionHeldProfit,pensionHeldReturn,orderedPensionRows}=pensionSectionInputs(x);
  return `<div class="grid two asset-detail-grid pension-asset-detail-grid">${renderPensionProductsBlock(x,pensionCashCost,pensionHeldCost,pensionHeldProfit,pensionHeldReturn)}${renderPensionChangeBlock(x,orderedPensionRows)}</div>`;
}
function renderPension(x){
  return `<section id="pension-section"><div class="section-title"><h2><span class="section-title-icon" data-section-title-icon="briefcase" aria-hidden="true"></span>퇴직연금 현황</h2></div><div class="pension-band">${renderPensionOverview(x)}${renderPensionAssetDetail(x)}${renderPensionCharts(x)}</div></section>`;
}
...
export {
  renderPension,
  renderPensionAssetDetail,
  renderPensionOverview
};
