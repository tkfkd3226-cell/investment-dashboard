...
const PENSION_CLIENT_SESSION_KEY='investment-dashboard:pension-client-session:v1';
function pensionStorage(kind){
  try{return kind==='session'?window.sessionStorage:window.localStorage}catch(_){return null}
}
function pensionClientSessionId(){
  const storage=pensionStorage('session');
  let id='';
  try{id=String(storage?.getItem(PENSION_CLIENT_SESSION_KEY)||'')}catch(_){}
  if(id)return id;
  ...
  id=`pension-session-${uuid}`;
  try{storage?.setItem(PENSION_CLIENT_SESSION_KEY,id)}catch(_){}
  return id;
}
function readPensionPendingIdentityStore(){
  const storage=pensionStorage('local');
  let parsed={single:[],batch:[]};
  try{
    const raw=storage?.getItem(PENSION_PENDING_IDENTITY_STORAGE_KEY)||'';
    if(raw){const value=JSON.parse(raw);if(value&&typeof value==='object')parsed=value}
  }catch(_){}
  const now=Date.now();
  const fresh=list=>(Array.isArray(list)?list:[]).filter(v=>v&&Number(v.savedAt||0)>0&&now-Number(v.savedAt)<PENSION_PENDING_IDENTITY_TTL_MS).slice(-30);
  return {single:fresh(parsed.single),batch:fresh(parsed.batch)};
}
function writePensionPendingIdentityStore(store){
  const storage=pensionStorage('local');
  if(!storage)return;
  try{storage.setItem(PENSION_PENDING_IDENTITY_STORAGE_KEY,JSON.stringify(store))}catch(_){}
}
function findReusablePendingIdentity(list,fingerprint){
  const sessionId=pensionClientSessionId();
  const now=Date.now();
  return [...(list||[])].reverse().find(v=>{
    if(String(v?.fingerprint||'')!==String(fingerprint||''))return false;
    if(String(v.ownerSessionId||'')===sessionId)return true;
...
function markPensionPendingSingleStatus(fingerprint,id,status,payload=null){
  ...
  const now=Date.now();
  let found=false;
  store.single=store.single.map(v=>{
    if(String(v?.fingerprint||'')!==String(fingerprint||'')||String(v?.id||'')!==String(id||''))return v;
    found=true;
...
function clearPensionPendingSingle(fingerprint,id){
  const store=readPensionPendingIdentityStore();
  store.single=store.single.filter(v=>!(String(v?.fingerprint||'')===String(fingerprint||'')&&String(v?.id||'')===String(id||'')));
  writePensionPendingIdentityStore(store);
}
function pensionBatchQueueIdentitySignature(queue=pensionEditorState.batchQueue){
  return (queue||[]).map(pensionBatchOperationFingerprint).join('\n');
}
function findPendingBatchIdentity(signature){
  const store=readPensionPendingIdentityStore();
...
function clearPendingBatchIdentity(signature,batchRequestId){
  const store=readPensionPendingIdentityStore();
  store.batch=store.batch.filter(v=>!(String(v?.fingerprint||'')===String(signature||'')&&String(v?.batchRequestId||'')===String(batchRequestId||'')));
  writePensionPendingIdentityStore(store);
}
...
<div class="pension-contrib-context" aria-label="퇴직연금 금액 조정 옵션">
  <div class="pension-contrib-context-head"><span class="contrib-field-label">조정 항목</span><div class="control-segmented pension-work-mode" role="group" aria-label="처리 방식 선택"><button type="button" class="pension-work-mode-btn active" aria-pressed="true" data-mode="single" data-pension-action="set-batch-mode" data-pension-enabled="false">개별 처리</button><button type="button" class="pension-work-mode-btn" aria-pressed="false" data-mode="batch" data-pension-action="set-batch-mode" data-pension-enabled="true">작업 모음 <span id="pensionBatchModeCount">0</span></button></div></div>
...
<div class="pension-contrib-tool modal-card-box" role="group" aria-label="등록">
  <div id="pensionContribStandardFields" class="pension-adjust-form cash-mode">
    ...
    <div class="contrib-field pension-cash-memo-field"><label for="pensionContribMemo">메모</label><span class="pension-control-shell"><input id="pensionContribMemo" type="text" value="현금성자산 앱 확인" data-contrib-default-memo="${contribDefaultMemo}" data-cash-default-memo="현금성자산 앱 확인"></span></div>
  </div>
  <div id="pensionEtfTradeFields" class="pension-etf-trade-fields" hidden>
    <div class="pension-adjust-form trade-mode">
      <div class="contrib-field pension-trade-date-field"><label for="pensionEtfTradeDate">신청일</label><span class="pension-control-shell pension-date-shell"><input id="pensionEtfTradeDate" type="date" value="${cashDefaultDate}" data-pension-change="trade-preview"></span></div>
      <div class="contrib-field pension-trade-product-field"><label for="pensionEtfTradeTicker">ETF 상품</label><span class="pension-control-shell pension-select-shell"><select id="pensionEtfTradeTicker" data-pension-change="trade-preview">${pensionTradeProductOptions()}</select></span></div>
...
const isSafePensionWhole=(value,{positive=false}={})=>Number.isSafeInteger(value)&&(positive?value>0:value>=0);
function assertSafePensionAggregate(values){
  for(const [label,value] of Object.entries(values||{})){
    if(!Number.isFinite(value)||Math.abs(value)>Number.MAX_SAFE_INTEGER)throw new Error(`${label} 계산값이 안전한 정수 범위를 벗어납니다.`);
  }
  return values;
}
...
function pensionEtfTradeExpectedForMode(draft,batchMode){
  ...
  if(!product||!isSafePensionWhole(qty,{positive:true})||!isSafePensionWhole(amount,{positive:true})) return null;
  try{
    ...
    assertSafePensionAggregate({현금성자산:cashBefore,'거래 후 현금성자산':cashAfter,'보유 수량':state.qty,'거래 후 보유 수량':qtyAfter,'취득원가':state.cost,'거래 후 취득원가':costAfter});
    return {
      state,
      cashBefore,
      cashAfter,
    ...
    return {error:error?.message||'연금 계산값을 확인할 수 없습니다.'};
  }
}
...
function buildPensionContributionItem(){
  ...
  if(target==='etfTrade'){
    ...
    if(draft.qtyRaw===''||!isSafePensionWhole(draft.qty,{positive:true})) throw new Error('체결수량을 안전한 정수로 입력해주세요.');
    if(draft.amountRaw===''||!isSafePensionWhole(draft.amount,{positive:true})) throw new Error('체결금액을 안전한 정수로 입력해주세요.');
  ...
  if(target==='cashSnapshot'){
    if(rawAmount===''||!isSafePensionWhole(amount)) throw new Error('평가금액을 안전한 정수로 입력해주세요.');
    const rawCostBasis=String(cashCostEl?.value||'').trim();
    const costBasis=cleanNumberInput(rawCostBasis);
    if(rawCostBasis===''||!isSafePensionWhole(costBasis)) throw new Error('매수원금을 안전한 정수로 입력해주세요.');
    return {target,date,valuation:amount,costBasis,memo};
  }
  if(rawAmount===''||!isSafePensionWhole(amount,{positive:true})) throw new Error('금액을 안전한 정수로 입력해주세요.');
...
function pensionBatchCashAvailable(state,asOfDate){
  ...
  if(snapshot){
    ...
    return Math.max(0,assertSafePensionAggregate({현금성자산:balance}).현금성자산);
  }
...
function pensionBatchPositionState(pos,d,state){
  ...
  assertSafePensionAggregate({'보유 수량':qty,'취득원가':cost,'실현손익':realizedProfit});
...
function pensionBatchSimulate(operations=pensionEditorState.batchQueue){
  ...
  orderInfo.operations.forEach((op,index)=>{
    ...
    const item={...(op.item||{})};
    if(op.target==='cashSnapshot'){
      if(!/^\d{4}-\d{2}-\d{2}$/.test(String(item.date||''))||!isSafePensionWhole(Number(item.valuation))||!isSafePensionWhole(Number(item.costBasis)))throw new Error(`${index+1}번 현금성자산 저장값을 확인해주세요.`);
      const saved={
        ...item,
    ...
    if(op.target==='contribution'){
      if(!/^\d{4}-\d{2}-\d{2}$/.test(String(item.date||''))||!isSafePensionWhole(Number(item.amount),{positive:true}))throw new Error(`${index+1}번 기업적립금 저장값을 확인해주세요.`);
      const saved={...item,id:item.id||op.tempId||`batch-contrib-${index}`,date:String(item.date),amount:Number(item.amount),updatedAtKST:`batch-${String(index).padStart(4,'0')}`};
    ...
    const qty=Number(item.qty),amount=Number(item.amount);
    if(!/^\d{4}-\d{2}-\d{2}$/.test(String(item.tradeDate||''))||String(item.tradeDate)>today)throw new Error(`${index+1}번 추가 매수 신청일을 확인해주세요.`);
    if(!isSafePensionWhole(qty,{positive:true})||!isSafePensionWhole(amount,{positive:true}))throw new Error(`${index+1}번 추가 매수 수량·금액을 확인해주세요.`);
...
function preparePensionBatchCashPrecondition(operation){
  const op={...(operation||{})};
  if(op.target!=='cashSnapshot')return op;
  const date=String(op.action==='delete'?op.key:op.item?.date||'');
  const source=op.sourceItem||pensionCashSnapshotItems().find(v=>String(v.date||'')===date)||null;
  if(op.action==='delete'){
    const expectedVersion=pensionCashSnapshotVersion(source);
    if(!expectedVersion)throw new Error('삭제 대상 현금성자산 버전을 확인하지 못했습니다. 새로고침 후 다시 시도해주세요.');
    op.expectedVersion=expectedVersion;
    op.expectedAbsent=false;
    return op;
  }
  if(source){
    op.expectedVersion=pensionCashSnapshotVersion(source);
    op.expectedAbsent=false;
  }else{
    op.expectedVersion='';
    op.expectedAbsent=true;
  }
  return op;
}
...
function getPensionBatchRequestId(){
  ...
  const signature=pensionBatchQueueIdentitySignature();
  const pending=findPendingBatchIdentity(signature);
...
function addPensionBatchOperation(operation){
  clearPensionContributionOutput();
  operation=preparePensionBatchCashPrecondition(operation);
  const now=Date.now();
  ...
  const operationUuid=(typeof crypto!=='undefined'&&typeof crypto.randomUUID==='function')?crypto.randomUUID():`${now}-${nextSequence}-${Math.random().toString(36).slice(2,10)}`;
  const op={...operation,qid:`batch-${now}-${nextSequence}`,tempId:`batch-temp-${now}-${nextSequence}`,logicalOperationId:String(operation.logicalOperationId||`batch-logical-${operationUuid}`)};
...
async function applyPensionBatchQueue(renderDashboard){
  ...
  const batchSignature=pensionBatchQueueIdentitySignature();
...
function preparePensionSingleSaveItem(item){
  ...
  if(pensionEditorState.singleSaveFingerprint!==fingerprint||!pensionEditorState.singleSaveId){
    ...
    pensionEditorState.singleSavePayload=item.target==='cashSnapshot'?{...prepared,requestId:id,logicalOperationId:id}:{...prepared,id:id,logicalOperationId:id};
    return {...pensionEditorState.singleSavePayload};
  }

  if(pensionEditorState.singleSavePayload)return {...pensionEditorState.singleSavePayload};
  const prepared={...item};
  const payload=item.target==='cashSnapshot'?{...prepared,requestId:pensionEditorState.singleSaveId,logicalOperationId:pensionEditorState.singleSaveId}:{...prepared,id:pensionEditorState.singleSaveId,logicalOperationId:pensionEditorState.singleSaveId};
...
async function savePensionContribution(){
  ...
  try{
    ...
    const saveFingerprint=pensionEditorState.singleSaveFingerprint;
    const saveIdentity=pensionEditorState.singleSaveId;
    ...
    if(!data){
      if(!saveAttempted)clearPensionPendingSingle(saveFingerprint,saveIdentity);
      clearPensionContributionOutput();
      return;
    }
    if(data.requiresDuplicateConfirmation===true){
