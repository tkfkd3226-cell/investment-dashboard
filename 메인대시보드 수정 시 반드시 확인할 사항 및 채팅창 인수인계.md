...
- 공통 helper
- 데이터 loading 관련 기반
- 여러 모듈에서 공통으로 사용하는 저수준 DOM/UI helper의 제한적 예외

원칙적으로 이 파일은 **데이터·state·계산·formatter 중심의 core 계층**으로 유지한다.

다만 현재 구조처럼 `dashboard-ui.js`, `dashboard-charts.js`, `dashboard-pension.js` 등 여러 모듈에서 공통으로 사용하는 **저수준 DOM/UI helper**는 중복 방지와 의존성 단순화를 위해 예외적으로 둘 수 있다. 대표적으로 modal/dialog의 focus 이동, focus-trap, focus 복귀처럼 특정 화면의 렌더링과 무관한 공통 접근성 helper가 이에 해당한다.
...
같은 **화면별 렌더링, selector 기반 UI 제어, class/innerHTML 조작 등 기능별 DOM 로직**을 `dashboard-core.js`에 넣지 않는다.

저수준 DOM/UI helper를 새로 추가해야 한다면, 실제로 여러 모듈에서 공통 사용되는지와 기존 모듈 책임 경계를 먼저 확인하고 최소 범위에서만 예외로 허용한다.
...
- 디자인 token 무시
- 메인 JS 책임 경계 파괴
- `dashboard-core.js`에 화면별 DOM 조작 또는 공통성이 없는 기능별 UI 로직 추가
...
# 61. 이 문서는 최신 ZIP 내부에서 자동으로 읽는 단일 운영·인수인계 기준 문서다
...
# 96. 새 채팅에서 사용할 최소 요청문

앞으로는 최신 전체 ZIP 하나에 코드와 이 MD가 함께 들어 있으므로 별도 인수인계문이 필요 없다.
...
사용자가 ZIP을 첨부하면서 바로 구체적인 수정 요청을 했다면, 별도의 시작 확인 문구를 요구하지 말고 같은 절차를 내부적으로 먼저 수행한 뒤 요청을 처리한다.
...
> **새 채팅에서는 최신 전체 ZIP 하나만 첨부하고, GPT는 ZIP 내부의 `메인대시보드 수정 시 반드시 확인할 사항 및 채팅창 인수인계.md`를 가장 먼저 읽은 뒤 같은 ZIP의 실제 소스를 source of truth로 확인한다. 차수 수정은 최소 변경과 최소 안전검사만 수행하고 QA는 별도 요청에서 검증하며, 기본 QA 점수는 변동 항목만 보여주되 사용자가 `모든 점수`를 요청하면 전체 점수표를 생략 없이 표시한다.**
