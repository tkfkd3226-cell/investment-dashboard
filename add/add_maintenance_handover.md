...
- `calc`와 `report`의 모서리는 메인 대시보드 기본 Corner 체계와 **같은 cap 값**을 사용한다. `add/css/common.css`의 기준값은 `surface 6px / control 5px / inner 4px`이다.
- 카드·패널·section 등 surface는 `--corner-surface-cap`, 버튼·탭·입력 등 control은 `--corner-control-cap`, hero 내부 chip·formula 같은 inner surface는 `--corner-inner-cap`을 우선 사용한다. 원형 아이콘·donut·dot와 의도적인 pill/badge는 예외다.
