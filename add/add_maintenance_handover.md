...
- **Calc 상단 입력카드의 Web/Tablet column composition은 동일하게 유지한다.** `보유 중 추가매수`와 `이전 거래 후 재매수`는 3개 카드를 동일폭 3열로, `이전 거래 없음`은 `현재 보유분`과 `계산 기준`을 동일폭 2열로 표시한다. Tablet에서는 density만 조정하고 열 수·가로 비율을 다시 바꾸지 않으며, Phone에서만 모든 입력카드를 1열로 재배치한다. 날짜형 빠른 매수 shortcut 삭제 후 생긴 빈 grid track이나 margin 보정으로 상단 여백을 만들지 않는다.
