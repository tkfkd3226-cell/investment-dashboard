...
- **Calc의 주요 선택 버튼은 하나의 Choice Group primitive를 공유한다.** 상단 거래유형(`보유 중 추가매수 / 이전 거래 후 재매수 / 이전 거래 없음`), `계산 기준`, 하단 매도 전략 탭은 `calc-choice-group` / `calc-choice-button`의 동일한 높이·padding·Corner·typography·active 상태 contract를 Web/Tablet/Phone에서 공통 사용한다. 거래유형에 따른 차이는 하단 전략의 3열/2열 구성만 허용하고 viewport별 feature selector로 버튼 크기를 다시 분기하지 않는다.
