...
- **Calc control 외곽 geometry도 단일 token을 Source of Truth로 사용한다.** 일반 input·date shell/inner control·계산 기준 선택버튼은 `--calc-control-box-height` 계열에서 같은 높이와 border 기준을 파생하고, feature selector에 동일한 `calc(...)` 높이식을 복제하지 않는다. 일반 input은 `calc-control-shell`, date는 기존 `date-control-shell`이 외곽 geometry/focus/error surface를 소유한다. Phone에서는 iOS focus zoom을 막기 위해 focus 가능한 input의 computed font-size를 16px로 유지하되 shell 내부 control만 optical scale하여 Web/Tablet과 같은 canonical visual value size로 표시한다. `maximum-scale` 같은 viewport 확대 차단으로 대체하지 않는다.
- **Calc의 주요 선택 버튼은 `calc-choice-button`의 공통 시각 contract를 공유하되 가로 layout은 공통화하지 않는다.** 상단 거래유형, `계산 기준`, 하단 매도 전략은 높이·세로 padding·Corner·typography·active 상태를 Web/Tablet/Phone에서 동일하게 사용한다. 반면 버튼 가로폭·열 구성·그룹 전체 폭은 각각 `preset-bar` / `seg` / `tabs`가 소유하며, 상단 거래유형의 자연폭/Phone 3열, 계산 기준의 카드 내부 3등분, 하단 전략의 Desktop/Tablet 자연폭 및 Phone 3열·거래유형별 2열 전환을 서로 억지로 합치지 않는다.
- **Calc의 중간 계산 요약은 하나의 공통 outer surface로 묶고 내부 정보 위계만 4개 KPI grid + 3개 range grid로 나눈다.** 두 행을 각각 별도 큰 카드로 다시 분리하거나 상단 입력카드 안에 편입하지 않는다. 중간 요약 outer surface의 내부 padding은 상단 입력 큰 카드와 하단 전략 본문이 사용하는 동일 large-surface density contract를 공유하고, Phone에서도 세 영역이 함께 같은 compact padding으로 전환한다. 내부 KPI/range 미니카드는 각 semantic surface를 유지하며, viewport에서는 내부 grid만 재배치한다.
