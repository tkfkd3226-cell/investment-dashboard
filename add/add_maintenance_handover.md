...
- **Calc control 외곽 geometry와 내부 spacing도 단일 token을 Source of Truth로 사용한다.** 일반 input·date shell/inner control·주요 text/choice/step 버튼은 `--calc-control-box-height`와 `--calc-control-space`에서 같은 높이·border·8px 내부 여백을 파생하고, feature selector에 별도 padding 값을 복제하지 않는다. 일반 input은 `calc-control-shell`, date는 기존 `date-control-shell`이 외곽 geometry/focus/error surface를 소유한다. `원/%` 단위가 input 안에 겹쳐 표시되는 필드의 오른쪽 reserve와 고정 크기 icon/help button의 `padding:0`은 기능적 geometry 예외다. Phone에서는 iOS focus zoom을 막기 위해 focus 가능한 input의 computed font-size를 16px로 유지하되 shell 내부 control만 optical scale하고 pre-scale padding을 보정해 실제 visual padding 8px을 유지한다. `maximum-scale` 같은 viewport 확대 차단으로 대체하지 않는다.
- **Calc의 주요 선택 버튼은 `calc-choice-button`의 공통 시각 contract를 공유하되 가로 layout은 공통화하지 않는다.** 상단 거래유형, `계산 기준`, 하단 매도 전략은 높이·상하좌우 8px padding·Corner·typography·active 상태를 Web/Tablet/Phone에서 동일하게 사용한다. 반면 버튼 가로폭·열 구성·그룹 전체 폭은 각각 `preset-bar` / `seg` / `tabs`가 소유하며, 상단 거래유형의 자연폭/Phone 3열, 계산 기준의 카드 내부 3등분, 하단 전략의 Desktop/Tablet 자연폭 및 Phone 3열·거래유형별 2열 전환을 서로 억지로 합치지 않는다.
- **Calc의 중간 계산 요약은 하나의 공통 outer surface로 묶고 내부 정보 위계만 4개 KPI grid + 3개 range grid로 나눈다.** 두 행을 각각 별도 큰 카드로 다시 분리하거나 상단 입력카드 안에 편입하지 않는다. Calc의 카드·패널·결과표 cell은 `--calc-surface-space`를 단일 Source of Truth로 사용해 상하좌우 여백을 Desktop/Tablet/Phone 모두 8px로 동일하게 적용한다. 입력카드·KPI·range·전략 summary·Phone 결과카드 등 카드 사이 gap도 `--calc-card-gap`에서 같은 8px rhythm을 공유하며, 특정 카드만 별도 padding/gap 값을 만들지 않는다. 버튼·input 내부 padding도 별도 `--calc-control-space:8px` contract로 전 viewport 동일하게 관리하고, field label/control 간격·divider 보정처럼 역할이 다른 spacing은 카드 surface contract에 억지로 합치지 않는다.
...
- Calc 상세표의 열은 `table-layout:fixed`로 동일 폭을 유지한다. 표 종류별 `675px`·`870px` 같은 임의 `min-width`를 두지 않고, 현재 렌더된 label/value의 자연 폭 중 가장 넓은 셀을 기준으로 `가장 넓은 셀 폭 × 열 수`를 표 최소폭으로 계산한다. 표 cell의 상하좌우 padding도 Calc surface spacing과 동일한 Desktop/Tablet/Phone 모두 8px contract를 사용한다. 이 최소폭이 container를 넘는 경우에만 `add-table-scroll`에서 가로 스크롤하며 label/value를 압축·줄바꿈해 맞추지 않는다. viewport 변화로 typography가 달라질 수 있으므로 resize 시 같은 기준으로 다시 계산한다. 계산 로직과 Phone 카드 표현은 이 table presentation과 분리한다.
