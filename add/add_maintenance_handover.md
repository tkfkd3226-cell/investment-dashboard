...
현재 거래 리포트에 이미 존재하는 7/29 기준 자금 흐름은 **역사적 기준선**으로 유지한다.

기존 기준값의 의미를 새 거래의 자금 출처에 따라 다시 쪼개지 않는다.
...
새로운 거래가 7/29 이후 실현되었다면, 그 거래를 현금으로 했는지 신규 대출로 했는지와 관계없이 **순손익만 7/29 이후 실현손익에 추가**한다.

즉 향후 새 매매 때문에:

```text
총 운용원금에 신규 매수대금을 더하거나
신규 대출원금을 추가하거나
대출이자를 차감하는 식으로
자금 흐름 모델을 확장하지 않는다.
```

리포트 하단 또는 자금 흐름 설명에는 필요 시 다음 취지를 유지한다.

> 매매 자금의 조달 방식 및 대출이자 등 금융비용은 본 분석에서 제외한다.

### 7/29 이전 실현손익 중복 가산 금지

기존 리포트에서 7/29 이전 실현손익은 이미 자금 형성과정에 포함된 것으로 처리한다.

따라서 자금 흐름에서 다시 더하지 않는다.

이 기준은 사용자가 자금 흐름 산식을 별도로 변경하라고 하기 전까지 유지한다.
...
7. 7/29 이후 자금 흐름에는 새 순손익만 추가
8. 최신 증권사 원본 이미지 교체
9. calc.html이 canonical report 경로를 가리키는지 확인
10. 아래 QA 검산 수행
```

자금 출처 질문이나 대출이자 분석은 위 절차에 넣지 않는다.
...
[ ] 자금 흐름은 신규 자금 원천·대출이자 변수를 새로 만들지 않고 순손익만 반영
...
> **새 KODEX 레버리지 실현거래가 생기면 증권사 확정 순손익을 `data/portfolio.json`과 `add/report/kodex-leverage-report.html`에 함께 반영하고, 본 포지션/단타·차트·합계·원본근거·자금흐름을 동일 기준으로 갱신하되, 신규 매매의 자금 출처나 대출이자 등 금융비용은 추적하지 않으며 report 파일명은 날짜와 관계없이 고정한다.**
