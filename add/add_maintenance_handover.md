...
- **Calc의 중간 계산 요약은 하나의 공통 outer surface로 묶고 내부 정보 위계만 4개 KPI grid + 3개 range grid로 나눈다.** 두 행을 각각 별도 큰 카드로 다시 분리하거나 상단 입력카드 안에 편입하지 않는다. 아래 3개 상태 range 카드는 전체 면을 파랑·빨강·초록으로 채우지 않고 중립 control surface를 공유하며, 상태 구분은 semantic accent만 사용한다. Calc의 카드·패널·결과표 cell은 `--calc-surface-space`를 단일 Source of Truth로 사용해 상하좌우 여백을 Desktop/Tablet/Phone 모두 6px로 동일하게 적용한다. 입력카드·KPI·range·전략 summary·Phone 결과카드 등 카드 사이 gap도 `--calc-card-gap`에서 같은 6px rhythm을 공유하며, 특정 카드만 별도 padding/gap 값을 만들지 않는다. 버튼·input 내부 padding도 별도 `--calc-control-space:6px` contract로 전 viewport 동일하게 관리하고, field label/control 간격·divider 보정처럼 역할이 다른 spacing은 카드 surface contract에 억지로 합치지 않는다. 입력카드 내부 subsection은 카드 외곽 방향에 추가 좌우·하단 padding을 만들지 않고, 점선 divider와 맞닿는 방향에만 `--calc-surface-space`를 사용해 separator breathing room을 확보한다.
