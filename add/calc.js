...
(() => {
  ...
  function settledStrategyHTML(kind,d,c){
    ...
    return `<div class="panel strategy-card no-badge add-card-shell add-card-base add-card-shadow"><div class="strategy-head"><div class="strategy-title"><h2 class="add-heading-section">${title}</h2><p>${desc}</p></div></div><div class="strategy-body"><div class="summary-row">${summary}</div><div class="desktop-details"><div class="section-title add-heading-subsection">매도 결과</div>${desktopGrid(saleH,saleV,'five-grid')}<div class="section-title add-heading-subsection">${integrationTitle}</div>${desktopGrid(integratedH,integratedV,integrationGridClass)}<div class="section-title add-heading-subsection">${full?'매도 후 최종 상태':'원금 회수 후 보유 현황'}</div>${desktopGrid(remainH,remainV,full?'final-grid':'simple-grid')}</div><div class="mobile-data">${mobileRows('매도 결과',saleH,saleV)}${mobileRows(integrationTitle,integratedH,integratedV)}${mobileRows(full?'매도 후 최종 상태':'원금 회수 후 보유 현황',remainH,remainV)}</div>${warning}<div class="note">${note} 수수료·세금 등 거래비용 미반영.</div></div></div>`;
  }
