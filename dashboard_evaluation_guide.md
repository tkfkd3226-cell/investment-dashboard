...
- **실시간 시세 진입점**: Market AI 연결 확인 전/연결 해제에는 Web/Tablet Topbar와 Phone `관리` 메뉴의 `실시간 시세`가 모두 숨겨지고, 연결 중에는 공통 명칭/action token을 공유하고, Web/Tablet은 compact no-scroll scaled monitor, Phone은 fullscreen responsive monitor 계약을 따르는가.
