...
- **Pension latency instrumentation:** Single/Batch 정상·duplicate·stale·fail-closed·예외 응답에서 `timing` 진단 정보가 business 결과와 분리되어 붙는가. timing version 1의 `totalMs/measuredMs/unattributedMs`와 stage/detail duration은 음수가 아니며, Single cashSnapshot 경로에서 target snapshot·semantic ledger·exact identity·dependency/epoch·confirmation·commit preflight·GitHub commit·local finalize가 구분되는가. Batch에서는 receipt/durable identity/intent-dependency 준비와 state snapshot read, ledger/identity read, operation apply, commit, receipt/intent finalize가 구분되고 `details.stateRead`는 상위 `stateSnapshotRead`를 분해할 뿐 measured 합계에 이중 산입되지 않는가. 예외 timing도 `doPost` error JSON까지 전달되는가. 계측은 `Date.now()` 외 원격 I/O를 추가하거나 request identity, mutation epoch, optimistic concurrency, confirmation token, CAS/response-loss 복구 순서를 바꾸지 않는가. UI는 `timing` 존재 여부를 성공/실패나 사용자 문구에 사용하지 않고, `dashboard-pension-editor.js`가 응답의 `timing`만 `[Pension timing]` JSON 로그로 개발자 콘솔에 출력하며 PIN·저장 payload는 기록하지 않는가.
- **Pension Single immutable-ref read cache:** 성능 최적화용 request cache는 반드시 현재 요청 안에서만 생성되고 key가 `ref + path`인가. cache를 사용하는 ref는 `baseCommitSha` 같은 immutable commit SHA이며, 동일 ref/path의 target·semantic ledger·exact identity·dependency read만 재사용하는가. cashSnapshot upsert의 contribution/trade는 dependency fingerprint에서 읽힌 동일 base SHA 내용을 `cashDependencyRead`가 재사용할 수 있으나, 최신 branch HEAD 조회, `resolvePensionSingleCommitBase()`의 HEAD 변경 검증, precommit 실패 후 readback에는 과거 cache를 사용하지 않는가. optional ledger의 404 fallback이 required data read로 오인 재사용되지 않는가. cache 유무에 따라 dependency hash의 path 정렬·SHA 입력과 stale/identity/epoch/CAS 판정이 달라지지 않는가.
