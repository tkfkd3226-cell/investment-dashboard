...
- **KRX latency instrumentation:** 정상·skip·duplicate·fail-closed·예외 KRX 응답에서 `timing` 진단 정보가 business 결과와 분리되어 붙는가. timing version 2의 `totalMs`와 각 stage/detail duration은 음수가 아니고, `mode/outcome`이 실제 경로를 반영하며, `details.prepareDispatch`와 `details.persistSuccess`가 상위 stage 내부 write 비용을 분해하되 `measuredMs`에 중복 합산되지 않는가. 예외 timing도 `doPost` error JSON까지 전달되는가. 프론트는 timing을 성공/실패 판정이나 UI 문구에 사용하지 않고 `[KRX timing]` 콘솔 로그로만 남기는가. 계측을 추가하거나 수정할 때 `Date.now()` 호출 외에 원격 API 호출·Properties write·requestId 생성·state transition이 추가되지 않는가.
