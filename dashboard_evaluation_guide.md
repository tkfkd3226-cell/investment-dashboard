...
- **KRX local evidence write batching:** fresh dispatch 전 intent+operation marker와 dispatch 수락 뒤 durable completed CAS를 먼저 시도한 다음 accepted intent+run marker+completed receipt가 정상 여유 상태에서 `Properties.setProperties(..., false)` 1회로 저장되어 개별 Script Properties 왕복을 만들지 않는가. batch fast-path는 snapshot boundary 이후 같은 `ScriptLock` 안에서만 사용하고, 각 value 8KB budget·prefix cap·430KB global budget에 여유가 없으면 기존 개별 write/GC 경로로 fallback하는가. pre-dispatch batch가 응답 유실·부분 반영되면 exact read-back으로 두 key의 저장 여부를 확인하고 둘 다 확정되지 않은 경우 이전 raw 값으로 rollback한 뒤 POST를 보내지 않는가. dispatch 수락 뒤 local success batch가 부분 반영돼도 false failure/false success로 단정하지 않고, durable completed 또는 POST 전에 존재하던 active intent가 중복 dispatch barrier로 남아 복구 가능해야 하며, durable+receipt가 모두 실제 저장된 경우에만 active intent cleanup을 수행하는가. 명시적 4xx 재시도의 marker→durable inflight→retry intent causal 순서는 batching으로 재배열하지 않는가.
- **KRX latency instrumentation:** 정상·skip·duplicate·fail-closed·예외 KRX 응답에서 `timing` 진단 정보가 business 결과와 분리되어 붙는가. timing version 3의 `totalMs`와 각 stage/detail duration은 음수가 아니고, `mode/outcome`이 실제 경로를 반영하며, 정상 fresh path에서 `details.prepareDispatch.localPreDispatchBatchWrite`와 `details.persistSuccess.localSuccessBatchWrite / durableCompletedWrite / intentCleanup`이 상위 stage 비용을 분해하되 `measuredMs`에 중복 합산되지 않는가. fallback/rejected-retry detail도 실제 경로에 맞게 기록되는가. 예외 timing도 `doPost` error JSON까지 전달되는가. 프론트는 timing을 성공/실패 판정이나 UI 문구에 사용하지 않고 `[KRX timing]` 콘솔 로그로만 남기는가. 계측 자체는 `Date.now()` 외의 원격 호출을 추가하지 않으며, 성능 최적화로 추가된 batch write는 기존 local evidence의 의미·requestId 생성·CAS·state transition을 바꾸지 않는가.
