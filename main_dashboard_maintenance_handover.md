...
- 동일 KST 날짜·동일 보유 ticker universe에서 모든 ticker가 `state=closed + market_state=closed + usable=true + price>0`로 확정되면 그 closed snapshot을 오늘의 Market AI 최종 overlay로 유지한다. 이후 `/api/market-data/krx-quotes`는 같은 날짜/universe에 대해 네트워크 요청하지 않고 10초 interval은 날짜/universe 변경 여부만 로컬 sentinel으로 확인한다. 날짜가 바뀌거나 보유 universe가 달라지면 settled key가 달라져 자동으로 quote 조회를 재개한다. 현재 ticker가 0개인 경우에도 authoritative empty lease를 1회 성공적으로 전달한 뒤 같은 방식으로 network polling을 쉰다. 일부 ticker가 unusable이면 fully-closed로 보지 않아 polling을 계속하고 해당 ticker만 JSON fallback한다.
