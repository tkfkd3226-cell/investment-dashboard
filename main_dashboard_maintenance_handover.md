...
- 단건 저장은 frontend가 같은 요청의 실패·재시도 동안 identity를 재사용한다. 기업적립금·ETF 추가매수는 client-generated ID를, 현금성자산은 `requestId`를 사용한다. 응답이 확정되기 전 pending single identity는 PIN/인증값을 포함하지 않은 짧은 TTL `localStorage` store에 저장하고, 같은 탭 reload에서는 session identity로 즉시 복구하며 다른 탭에서는 uncertain 상태 또는 grace 이후에만 재사용한다. GAS는 최근 완료 receipt와 진행 intent에 더해 Script Properties의 단조 증가 `PENSION_MUTATION_EPOCH`을 같은 ScriptLock 아래에서 관리한다. 새 logical Pension mutation을 받아들이는 순간 epoch를 먼저 증가시키므로 GitHub write 성공/응답 유실 경계에서도 이후 작업이 시작되면 과거 retry는 stale이 된다. Git blob SHA는 내용이 원복될 때 과거 값으로 되돌아오는 ABA가 있으므로 causal version으로 사용하지 않고 target 외부 변경 감지의 보조 guard로만 사용한다. 같은 identity에 다른 내용이 오면 거부하며 stale retry는 `stale_retry_ignored` + `duplicate:true`로 mutation 없이 수렴한다.
...
- 현금성자산은 저장과 삭제 모두 optimistic concurrency precondition을 사용한다. 기존 snapshot을 수정/삭제할 때는 화면이 본 `expectedVersion`을, 해당 날짜 신규 생성은 `expectedAbsent:true`를 보내고 GAS가 현재 snapshot과 일치할 때만 mutation을 허용한다. 따라서 `화면 A가 100을 봄 → B가 200 저장 → 오래된 A가 새 requestId로 100 저장`이나 `삭제 A → 같은 날짜 snapshot B 저장 → 과거 삭제 A 재시도`가 최신 B를 되돌리거나 지울 수 없다. 단건 삭제는 멱등 DELETE로 취급하며 첫 삭제가 실제 반영된 뒤 응답만 유실된 재시도는 receipt/intent 또는 `delete_already_absent`로 안전하게 수렴한다.
- 단건 Pension 저장/삭제와 Batch는 같은 `ScriptLock` mutation boundary 및 `PENSION_MUTATION_EPOCH`을 공유한다. contribution/ETF 삭제가 연결 snapshot을 확인하는 동안 다른 단건 cashSnapshot 저장이 끼어 참조 관계를 깨뜨리는 cross-file race를 허용하지 않는다. Batch는 frontend의 `batchRequestId`와 각 작업의 stable `operationId`를 함께 전달하고, 응답이 확정되기 전에는 queue fingerprint + operationId 목록을 TTL pending store에 보존해 reload 후 동일 작업 모음 identity를 복구한다. cashSnapshot Batch upsert/delete는 queue를 구성할 때 본 initial snapshot의 `expectedVersion`/`expectedAbsent`를 operation에 묶고, GAS가 initial state에 대해 precondition을 한 번 검증한 뒤 같은 Batch 내부의 순차 변경은 허용한다. GAS는 commit 전에 `baseCommitSha`·mutation epoch·작업별 semantic dependency fingerprint를 intent에 기록한다. 이후 README/CSS처럼 Batch 의미와 무관한 branch commit만 앞서가면 dependency fingerprint가 동일한지 확인한 뒤 최신 HEAD를 새 base로 승격해 동일 요청을 재시도한다. 반대로 더 최신 Pension mutation이 시작됐거나 실제 dependency가 바뀌었으면 `batch_stale_retry_ignored`로 종료한다. receipt/cache 저장이 실패한 commit-success 경계에서는 cashSnapshot 날짜별 마지막 operation의 최종 효과와 contribution/ETF deterministic ID를 현재 GitHub state와 비교하여 duplicate로 수렴한다. Cache에는 과거 전체 pension state를 저장하지 않고 receipt형 요약만 저장한다.
- KRX 갱신은 frontend가 같은 실패·재시도 동안 `requestId`를 재사용하고 GAS가 dispatch 전에 request별 intent와 `branch + date/mode` operation marker를 durable하게 저장한 뒤 성공 receipt로 승격한다. receipt 저장/응답이 유실되어 intent만 남으면 동일 requestId는 `workflow_dispatch_uncertain`으로 중복 dispatch하지 않는다. 새 requestId가 와도 60초 visibility grace 동안 marker로 즉시 차단하고, 이후에는 workflow REST API의 queued/running 상태와 custom `run-name`을 확인해 같은 작업이면 `workflow_in_progress`로 수렴한다. `.github/workflows/update-prices.yml`은 같은 branch를 `concurrency + queue: max`로 직렬화해 pending 실행도 보존하고, push 직전 remote를 다시 fetch한다. checkout 이후 다른 commit이 branch를 앞서가면 KRX 관리파일뿐 아니라 generation input(`portfolio.json`, updater, requirements, workflow) 변경 여부도 확인한다. 계산과 무관한 commit만 최신 remote에 rebase하여 최대 3회 push 재시도하며, managed file 또는 generation input이 바뀌었으면 과거 계산 결과를 올리지 않고 fail-closed한다.
...
- Pending identity QA에서는 contribution/ETF 단건과 Batch가 응답 유실 후 같은 탭 reload에서 기존 requestId/batchRequestId/operationId를 복구하는지 확인하고, 다른 탭은 uncertain 상태 또는 grace 이후에만 같은 pending identity를 재사용하는지 확인한다. 성공 응답 뒤 pending record는 즉시 정리되어 이후 의도적인 동일 내용 신규 거래까지 막지 않아야 한다.
- Batch final-state recovery QA에서는 같은 날짜 cash를 한 Batch에서 `100 → 200`처럼 여러 번 upsert한 뒤 commit 성공 + receipt/cache/HTTP 응답 유실을 만들어, intermediate 100이 현재 state에 없더라도 날짜별 마지막 operation 200을 기준으로 `batch_duplicate_ignored`에 수렴하는지 확인한다.
