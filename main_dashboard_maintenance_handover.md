...
- 단건 저장은 frontend가 같은 요청의 실패·재시도 동안 identity를 재사용한다. 기업적립금·ETF 추가매수는 client-generated ID를, 현금성자산은 `requestId`를 사용한다. GAS는 최근 완료 receipt와 진행 intent에 더해 Script Properties의 단조 증가 `PENSION_MUTATION_EPOCH`을 같은 ScriptLock 아래에서 관리한다. 새 logical Pension mutation을 받아들이는 순간 epoch를 먼저 증가시키므로 GitHub write 성공/응답 유실 경계에서도 이후 작업이 시작되면 과거 retry는 stale이 된다. Git blob SHA는 내용이 원복될 때 과거 값으로 되돌아오는 ABA가 있으므로 causal version으로 사용하지 않고 target 외부 변경 감지의 보조 guard로만 사용한다. 같은 identity에 다른 내용이 오면 거부하며 stale retry는 `stale_retry_ignored` + `duplicate:true`로 mutation 없이 수렴한다.
...
- 단건 삭제는 멱등 DELETE로 취급한다. 현금성자산 삭제는 날짜만 보내지 않고 frontend가 stable `deleteRequestId`와 화면이 본 `expectedVersion`을 함께 전송하며, GAS가 현재 snapshot 버전과 일치할 때만 삭제한다. 따라서 `삭제 A → 같은 날짜 snapshot B 저장 → 과거 삭제 A 재시도`가 B를 지울 수 없다. 첫 삭제가 실제 반영된 뒤 응답만 유실된 재시도는 receipt/intent 또는 `delete_already_absent`로 안전하게 수렴하고, stale 응답에서는 프런트가 로컬 최신 상태를 임의 삭제하지 않는다.
- 단건 Pension 저장/삭제와 Batch는 같은 `ScriptLock` mutation boundary 및 `PENSION_MUTATION_EPOCH`을 공유한다. contribution/ETF 삭제가 연결 snapshot을 확인하는 동안 다른 단건 cashSnapshot 저장이 끼어 참조 관계를 깨뜨리는 cross-file race를 허용하지 않는다. Batch는 frontend의 `batchRequestId`와 각 작업의 stable `operationId`를 함께 전달하고, GAS는 commit 전에 `baseCommitSha`·mutation epoch·작업별 semantic dependency fingerprint를 intent에 기록한다. 이후 README/CSS처럼 Batch 의미와 무관한 branch commit만 앞서가면 dependency fingerprint가 동일한지 확인한 뒤 최신 HEAD를 새 base로 승격해 동일 요청을 재시도한다. 반대로 더 최신 Pension mutation이 시작됐거나 실제 dependency가 바뀌었으면 `batch_stale_retry_ignored`로 종료한다. receipt 저장이 실패한 commit-success 경계에서는 현재 GitHub state가 해당 batch 결과를 이미 반영했는지 먼저 확인하고 duplicate로 수렴한다. Cache에는 과거 전체 pension state를 저장하지 않고 receipt형 요약만 저장한다.
