...
- **기준 확인:** 새 채팅에서는 최신 전체 ZIP의 `main_dashboard_maintenance_handover.md`를 가장 먼저 읽고, 같은 ZIP의 실제 소스를 source of truth로 확인한다.
- **평가:** `점수`는 CSS·JS·UI·UX 세부점수와 각 총점·UI/UX 총점·전체 총점만 출력한다. `평가` 또는 `평가해줘`는 최신 실제 소스를 처음부터 독립적으로 전체 평가하며, 필요 시 canonical GitHub Pages를 평가용 runtime 보조 수단으로 사용한다.
- **수정·QA:** 수정은 현재 구조 안에서 최소 범위로 수행한다. QA는 변경 위험에 비례해 **현재 수정본 자체만** 검증하고 GitHub Pages 배포본을 근거로 사용하지 않으며, 여러 차수 작업은 마지막에 누적 최종 QA를 수행한다.
- **결과 전달:** 실제 변경 파일만 원래 경로를 유지한 ZIP으로 전달하고, GitHub 커밋용 짧은 Summary와 간단한 Description을 함께 제공한다.
- **문서·백엔드:** handover는 수정 이력을 자동 누적하지 않고, 장기 유지보수 contract가 실제로 바뀐 경우에만 기존 항목을 수정·통합한다. GAS는 GitHub 프로젝트와 별도로 운영하며, 서버 내부 검증은 최신 운영 GAS 소스가 별도 제공된 경우에만 수행한다.
