...
- 401px 이상에서는 기존 메모 텍스트 표시 구조를 유지하되, 계좌별 table은 wrapper 폭을 우선 사용하고 메모 셀만 필요 시 자연스럽게 줄바꿈한다.
