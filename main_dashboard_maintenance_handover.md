...
- 개인보기 도구는 Web/Tablet에서 공통 icon-button geometry를 사용한다. Tablet/Phone hamburger는 같은 menu source를 공유하지만 Tablet에서는 `.tablet-topbar-ui` 상태로 Topbar와 중복되는 `관리` 그룹과 `투자 계산기` 링크를 즉시 숨기고, Phone에서는 `관리` 그룹을 유지한다. Market AI 연결 토글은 Phone Topbar에 두지 않는다.
...
- Dashboard-side Market AI 사용 preference는 `dashboard-market-ai-client.js`가 소유한다. OFF 시 signal/live polling과 in-flight 적용을 중단하고 volatile quote를 제거해 저장 JSON으로 fallback하며, ON 시 즉시 refresh를 시도한다. preference와 server reachability는 별도 상태다. Web/Tablet은 계산기와 테마 사이의 icon action을 사용한다. Phone은 hamburger `관리` 그룹의 같은 Market AI 연결 action을 사용하고, Tablet에서는 해당 `관리` 그룹이 Topbar 중복으로 숨겨진다. 이 토글은 backend runtime을 시작·종료하지 않는다.
...
- Desktop/Tablet에서 Market AI panel은 Hero의 normal flow/grid 높이 계산에서 분리해 우측에 절대배치한다. `top / right / bottom`은 모두 기존 `--hero-pad`를 사용하므로 Hero 자체 padding을 줄이지 않고 상·우·하 외곽 여백을 동일하게 유지한다. Market AI 자체 높이는 Hero의 자연 높이를 늘리지 않으며, 왼쪽 title/pill은 Market AI 최대폭과 layout gap만큼 우측 영역을 예약한다. 좁은 Tablet에서 왼쪽 내용이 실제로 wrap되면 그 내용의 자연 높이만 Hero 높이에 반영한다. Phone에서는 이 Hero 배치 규칙을 사용하지 않고 같은 panel DOM을 dialog로 이동한다.
