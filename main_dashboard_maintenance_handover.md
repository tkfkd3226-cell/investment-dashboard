...
최신 전체 ZIP이 첨부된 모든 작업에서는 **작업 종류와 관계없이 ZIP 루트의 `메인대시보드 수정 시 반드시 확인할 사항 및 채팅창 인수인계.md`를 가장 먼저 전체 읽는다.**
...
평가해줘
→ 아래의 공통 전역 독립 평가 프로토콜 적용
→ CSS / JavaScript / UI / UX를 최신 실제 코드 기준으로 독립평가
→ 파일 수정 / ZIP 생성 안 함
...
최종 QA
→ 여러 차수 또는 누적 수정본을 처음부터 다시 누적 검증

수정
→ 직전 대화에서 합의된 수정사항만 최소 변경
→ 합의되지 않은 추가 개선을 끼워 넣지 않음
...
사용자가 명령을 짧게 보내더라도, **항상 이 문서 전체를 먼저 읽은 뒤 그 명령의 의미를 적용**한다.


# 4. `평가해줘` 공통 전역 독립 평가 프로토콜
사용자가 최신 전체 ZIP을 첨부한 상태에서 **`평가해줘`**라고 요청하면, 별도의 긴 평가 요청문을 다시 요구하지 않고 **이 섹션의 전체 프로토콜을 자동 적용**한다.
...
2. 같은 ZIP의 `메인대시보드 수정 시 반드시 확인할 사항 및 채팅창 인수인계.md`
3. 같은 ZIP의 `README.md`
...
# 7. 메인 CSS는 `css/style.css` 단일 파일 유지
...
# 8. 메인 CSS는 기능별 영역 안에서 반응형까지 함께 관리
...
# 17. 디자인 토큰과 CSS variable 우선 재사용
...
# 18. 메인 JavaScript는 현재 7파일 ES Module 구조 유지
...
# 22. `dashboard-ui.js`와 `dashboard-ui-common.js` 책임
...
# 26. Inline event handler 재도입 금지
...
# 27. Event handler에 비즈니스 로직을 과도하게 넣지 않는다
...
# 28. JavaScript에서 UI 스타일 직접 지정 최소화
...
# 30. 현재 ES Module 구조와 단일 entry를 유지한다
...
# 37. Calc 모바일 KPI 설명은 강제 ellipsis로 다시 자르지 않는다
...
# 48. 새로운 breakpoint / `!important` 발생 시 별도 보고
...
# 65. 이 문서는 최신 ZIP 내부에서 자동으로 읽는 단일 운영·인수인계 기준 문서다
...
# 67. 2026-08-18 JS 3차 리팩토링 최종 QA PASS 기준선
...
# 69. 현재 ES Module dependency graph
...
# 70. ES Module import / export 운영 규칙
...
# 71. `index.html` module entry와 cache bust 정책
...
# 72. 현재 JS state / initialization ownership
...
# 74. 최신 반응형 기준과 iPhone 데스크탑 웹사이트 요청
...
# 75. Section Title / ON·OFF 버튼 높이 최신 불변조건
...
재갱신은 선택된 날짜의 저장 상태를 먼저 확인한 뒤 종가 기준이 아니면 다시 반영합니다.
```
...
또한 **파일 수정 작업을 완료해 결과 파일을 전달할 때는 GitHub 커밋에 바로 사용할 수 있는 짧은 `Summary` 문구를 답변에 함께 제공한다.**

기본 형식:

```text
Summary: 변경 핵심을 1문장으로 간단히 작성
```
...
- 여러 파일을 수정했더라도 하나의 간결한 커밋 요약으로 작성한다.
...
# 84. 사용자가 `모든 점수`라고 하면 전체 점수표를 반드시 표시
...
# 90. 새 채팅 시작 시 GPT가 반드시 먼저 확인할 것
...
# 91. MD와 최신 ZIP이 서로 다르면 임의로 맞추지 않는다
...
# 93. UI/UX 점수 이력 ① `investment-dashboard-main 3.zip` 최초 기준
...
# 94. UI/UX 점수 이력 ② 2026-08-16 중간 QA 기준
...
# 95. UI/UX 점수 이력 ③ 17차 최종 100점 milestone
...
# 96. `100점`은 현재 소스의 영구 고정 점수가 아니다
...
# 100. 차수형 대규모 작업을 새로 시작할 때 MD에 남길 최소 정보
...
# 101. GitHub / 로컬 파일이 엇갈릴 때의 source of truth 원칙
...
# 103. JS 구조 리팩토링 차수 작업 · QA · 보고 운영 원칙
...
### 2026-08-19 CSS 1~4차 구조정리 최종 기준선
...
# 104. Google Apps Script(GAS) 운영 및 배포 원칙
...
> **새 채팅에서는 최신 전체 ZIP 하나만 첨부하고, GPT는 ZIP 내부의 `메인대시보드 수정 시 반드시 확인할 사항 및 채팅창 인수인계.md`를 가장 먼저 읽은 뒤 같은 ZIP의 실제 소스를 source of truth로 확인한다. GAS 수정·검증이 필요한 경우에는 별도로 제공된 최신 운영 GAS 소스를 source of truth로 사용한다. 차수 수정은 최소 변경과 최소 안전검사만 수행하고 QA는 별도 요청에서 직전 변경 범위를 검증하며, 여러 차수 작업은 마지막에 누적 최종 QA를 별도로 수행한다. 결과 파일은 실제 변경 파일만 원래 경로를 유지한 ZIP으로 전달하고 ZIP 내부와 무결성을 확인하며, 파일 수정 결과에는 GitHub 커밋용 간단한 Summary를 함께 제공한다. 사용자가 앞으로 반복 적용할 유지보수·수정·QA·파일전달 조건을 추가하거나 변경하면 별도 요청 없이 `main_dashboard_maintenance_handover.md`의 기존 관련 항목에 자동 반영한다. 기본 QA 점수는 변동 항목만 보여주되 사용자가 `모든 점수`를 요청하면 전체 점수표를 생략 없이 표시한다. 사용자가 최신 전체 ZIP을 첨부하고 `평가해줘`라고 요청하면 과거 점수나 최근 수정 체크리스트를 근거로 사용하지 않고 이 문서의 `평가해줘 공통 전역 독립 평가 프로토콜`에 따라 최신 실제 소스를 처음부터 독립 평가하며 파일은 수정하지 않는다.**
