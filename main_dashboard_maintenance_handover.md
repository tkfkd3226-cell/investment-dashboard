...
- Market AI server reachability는 `dashboard-market-ai.js`가 공통 connection event로 publish하고 Topbar/UI가 소비한다. 서버 연결이 검증된 동안에만 웹/태블릿 Topbar의 `실시간 시세` 버튼과 Phone `관리` 메뉴의 동일 명칭 항목을 노출한다. 두 진입점은 `REALTIME_QUOTES_ACTION`과 `MARKET_AI_MONITOR_URL`을 공유한다. Web/Tablet은 iframe을 축소하지 않고 Monitor가 전달한 실제 콘텐츠 높이에 맞춰 compact·no-scroll modal을 사용하며, Phone UI는 외곽 여백 0의 fullscreen responsive iframe을 사용한다. 화면 크기 변경에도 이 구분과 no-scroll 계약을 유지하며, 연결 해제 시 진입점을 숨기고 열린 embedded monitor를 닫아 stale UI를 남기지 않는다.
