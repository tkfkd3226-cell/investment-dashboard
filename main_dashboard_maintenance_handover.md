...
- 401px 이상에서는 메모 텍스트를 표시하되 고정 최소폭/`nowrap`을 강제하지 않고, 가용폭이 부족해지는 시점부터 자연스럽게 줄바꿈한다.
