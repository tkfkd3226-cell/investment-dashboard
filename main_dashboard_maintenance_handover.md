...
- `scripts/update_prices.py`는 KST 오늘 `09:00 ≤ 시각 < 15:30`에 기존 pykrx 기본 시세(Naver)를 사용하고, **15:30 정각부터** 및 과거 거래일은 raw KRX 종가 경로를 사용한다. 주식은 `get_market_ohlcv_by_ticker(date, market="ALL", alternative=False)` → `get_market_ohlcv_by_date(..., adjusted=False)`, ETF는 `get_etf_ohlcv_by_ticker(date)` → `get_etf_ohlcv_by_date(...)`로 분리한다. 주식 ALL에는 ETF가 포함되지 않으며 전종목 캐시는 `(날짜, ETF 여부)`로 구분한다. Naver/NXT/애프터마켓 공개 현재가·dayCandle을 정규장 종가 fallback으로 사용하지 않는다. 성공한 close에는 `regularCloseSource: pykrx_raw`(기존 valuation override 포함 시 `pykrx_raw+nxt_valuation_override`)를 기록한다. 해당 날짜 종가가 아직 제공되지 않거나 인증·조회가 실패하면 오류를 내고 원본 JSON을 유지한다. 조회 도중 15:30을 넘으면 전체 종목을 1회 다시 조회하여 장중 가격이 종가 라벨로 저장되는 일을 막는다. Actions 실행 시점이 가격 기준이며 클릭 시점 가격을 예약하는 기능은 아니다.
- **선택일 재갱신은 기존 종가 라벨에 관계없이 실제 dispatch를 허용**한다. 자동 최신/누락 반영에서만 `priceBasis:regular_close`와 `regularCloseSource:pykrx_raw` 계열 검증을 이용해 이미 확정된 종가의 불필요한 실행을 생략한다. 기존 requestId/durable ledger idempotency와 진행 중 작업 중복 방지는 두 모드 모두 유지한다.
