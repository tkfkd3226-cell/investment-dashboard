...
- Market AI server reachability는 `dashboard-market-ai.js`가 공통 connection event로 publish하고 Topbar/UI가 소비한다. 서버 연결이 검증된 동안에만 웹/태블릿 Topbar의 `실시간 시세` 버튼과 Phone `관리` 메뉴의 동일 명칭 항목을 노출한다. 두 진입점은 `REALTIME_QUOTES_ACTION` 한 토큰을 공유하고 `dashboard-market-ai-client.js`의 `MARKET_AI_MONITOR_URL`을 사용한다. modal은 공통 `--modal-frame-wide-width:1280px` / `--modal-frame-wide-height:720px` frame token을 사용하되 작은 viewport에서는 overlay 안쪽으로 clamp한다. 연결 해제 시 진입점을 숨기고 열린 embedded monitor를 닫아 stale UI를 남기지 않는다.
