...
- `scripts/update_prices.py`의 당일 장중 조회는 기존 pykrx 기본 경로를 유지한다. `market_status_for_date(target_date)==close`인 장마감/과거일은 **`pykrx adjusted=False`의 KRX 원천 일봉 종가만** `regular_close`로 인정한다. Naver 국내종목 `dayCandle` 등 공개 quote/chart endpoint는 NXT·시간외 가격이 일봉 종가처럼 노출될 수 있으므로 개별종목 정규장 종가 fallback으로 사용하지 않는다. raw pykrx가 해당 날짜 종가를 확인하지 못하면 이전값이나 공개 시세를 `regular_close`로 승격하지 않고 fail-closed한다. 성공한 close snapshot에는 `regularCloseSource: pykrx_raw`(거래 event의 valuation override가 섞인 경우 `pykrx_raw+nxt_valuation_override`)를 기록하며, 이 source attestation이 없는 기존 `regular_close`는 GAS가 한 번 재확정한다. 명시 날짜 검증에서는 실제 비거래일(직전 거래일 확인 가능)과 데이터 취득 실패를 다른 오류로 구분한다.
