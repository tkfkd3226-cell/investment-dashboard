...
- `scripts/update_prices.py`의 당일 장중 조회는 기존 pykrx 기본 경로를 유지한다. `market_status_for_date(target_date)==close`인 장마감/과거일은 **raw KRX 데이터만** `regular_close`로 인정한다. 우선 `get_market_ohlcv_by_ticker(date, market="ALL", alternative=False)`의 해당 날짜 전종목 원시 종가를 한 번 받아 날짜별로 캐시하고, 이 KRX endpoint가 실패한 경우에만 `get_market_ohlcv_by_date(..., adjusted=False)`의 raw 개별종목 일봉을 failover로 사용한다. Naver 국내종목 `dayCandle` 등 공개 quote/chart endpoint는 NXT·시간외 가격이 일봉 종가처럼 노출될 수 있으므로 개별종목 정규장 종가 fallback으로 사용하지 않는다. 두 raw KRX 경로가 모두 해당 날짜 종가를 확인하지 못하면 이전값이나 공개 시세를 `regular_close`로 승격하지 않고 fail-closed한다. 성공한 close snapshot에는 `regularCloseSource: pykrx_raw`(거래 event의 valuation override가 섞인 경우 `pykrx_raw+nxt_valuation_override`)를 기록하며, 이 source attestation이 없는 기존 `regular_close`는 GAS가 한 번 재확정한다. 명시 날짜 검증에서는 실제 비거래일(직전 거래일 확인 가능)과 데이터 취득 실패를 다른 오류로 구분한다.
- GAS는 `marketStatus:close`나 `priceBasis:regular_close`만으로 중복 실행을 차단하지 않는다. **`priceBasis:regular_close`이면서 `regularCloseSource`가 `pykrx_raw` 계열로 검증된 snapshot**만 `이미 정규장 종가 기준 데이터가 반영되어 있습니다.`로 차단한다. `priceBasis`가 없는 legacy close와 source attestation 없는 과거 `regular_close`는 당일 장마감 후 또는 선택일 재갱신에서 한 번 재확정할 수 있어야 한다. 이 변경은 기존 requestId/durable ledger idempotency를 제거하는 것이 아니며 `reconfirm_regular_close`도 durable decision reason으로 기록한다.
...
전량매도 거래 상세 tooltip도 위 취소선과 **동일한 상태 조건**을 사용한다. `securityFullExitSaleForDate(ticker, date)`가 현재 전량매도 lifecycle을 만든 최신 매도일의 `securitiesEvents`를 합산하고, 공통 `securitySaleTooltipAttrs()`가 `매도가 / 거래비용 / 순매도대금 / 매수원가 / 실현손익`을 data attribute로 전달한다. `전일 대비 변동 > 종목 셀`, `보유종목 현황 > 종목 셀`, `종목별 누적손익`의 해당 종목 카드 전체, `평가금액 비중 > 종목별`의 해당 종목 카드 전체가 같은 `securitySaleTooltip` UI와 pointer/focus/Esc/scroll/resize lifecycle을 공유한다. 선택일이 매도일 이후여도 historical 카드에는 최신 전량매도 이벤트의 실제 원장값을 표시하며, 재매수되면 취소선과 tooltip이 함께 해제된다. 종목명·티커·날짜·tooltip 수치는 하드코딩하지 않는다.
