...
현재 실제 구현에서는 **400px 이하**에서 계좌별 성과 메모 헤더를 시각적으로 `i`로 축약하고, 각 메모를 작은 원형 정보 버튼으로 전환한다. 접근성상 헤더의 의미는 계속 `메모`로 유지한다.
