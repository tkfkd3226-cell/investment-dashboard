...
라고 하면 **직전 수정 결과만 검증한다.**

QA에서는 원칙적으로 파일을 수정하지 않고, 전체 프로젝트를 다시 분석하지 않는다. 검증 범위는 **직전 변경의 영향도와 위험도에 비례**해 정한다.
...
모든 `QA`를 무조건 전체 프로젝트 QA로 확대하지 않는다.
...
다음과 같은 요청은 단순 `QA`보다 넓은 **전체 수준 검증 명령**으로 해석한다.
...
이 경우에만 필요에 따라 검증 범위를 다음까지 확대한다.
...
## 3.16 차수형 대규모 작업을 새로 시작할 때 MD에 남길 최소 정보
...
## 3.17 대규모 JS 구조 작업의 차수 · QA · 보고 운영
...
### 3.18.2 사용자의 요청이 현재 구조를 훼손하면 작업 중지
...
### 3.18.4 새 코드 적용 후 불필요한 예전 코드 제거
...
### 3.18.9 새로운 breakpoint / `!important` 발생 시 별도 보고
...
### 3.18.11 계산 로직 수정은 UI보다 엄격하게 검증
...
### 3.18.14 구조 보존을 이유로 과도하게 소극적이지 않는다
