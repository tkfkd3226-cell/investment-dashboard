...
- 로컬 UI 점검용 `?marketAiPreview=1`은 localhost뿐 아니라 사설 LAN IP(10.x / 172.16~31.x / 192.168.x / 169.254.x)와 `.local` 호스트에서도 동작한다. 또한 기존 canonical 상태인 `html.iphone-request-desktop`에서는 별도 query 없이 같은 preview fallback을 허용해 iPhone의 '데스크탑 웹사이트 요청'에서도 Desktop Market AI를 확인할 수 있다. API 실패/404/stale일 때만 DB 응답 형태의 preview 값을 사용하고 정상 API 응답이 있으면 실제 데이터를 우선한다. 일반 Mobile/실제 터치폰 가로 숨김 정책은 유지하며, 일반 폰에서 query preview를 명시한 경우에만 2×2 metric layout으로 UI를 확인한다. 실제 Market AI DB에는 UI 점검용 가짜 row를 넣지 않는다.
