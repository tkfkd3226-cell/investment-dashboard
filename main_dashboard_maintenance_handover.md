...
- Main의 공통 section/chart 제목(`.section-title h2/h3`, `.chart-head h3`)은 `--section-title-line-height:1`을 사용한다. 제목 텍스트 line-box, 왼쪽 `.section-title-icon`, 선택적 info slot이 같은 높이 기준에서 `align-items:center`로 정렬되도록 하고, 과거 `.section-title-icon{margin-bottom:1px}` 같은 광학 보정은 두지 않는다. Desktop/Tablet/Phone 모두 이 공통 계약을 따르며 다른 역할의 Modal/인사이트/데이터카드/Add 제목에는 전파하지 않는다.
