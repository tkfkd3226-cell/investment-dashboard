...
- KRX active-run 확인은 누적된 completed 이력을 1~10페이지 순차 검색하지 않는다. `queued / in_progress / waiting / pending / requested` 상태만 GitHub `status` filter로 좁혀 `UrlFetchApp.fetchAll()` 한 번에 병렬 조회한다. 각 active status가 100건을 넘어 1페이지로 전체 범위를 확정할 수 없거나 어느 조회가 실패하면 **새 dispatch를 허용하지 않고 `workflow_status_uncertain`으로 fail-closed**한다. 과거 uncertain requestId의 정확한 acceptance proof를 복구하는 예외 경로만 exact run-name historical search를 유지한다. dispatch 성공 후 신규 durable completed entry를 만들 때는 이미 읽은 HEAD+shard context에서 바로 CAS write하고, CAS 충돌이 난 경우에만 최신 HEAD/shard를 다시 읽는다.
