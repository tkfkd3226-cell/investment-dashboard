...
- Market AI의 시장/신호 두 카드는 Web·Tablet·Phone 모두 `13:7` 같은 고정 비율을 사용하지 않는다. 카드 바깥 폭은 공통 `content-driven` 방식으로 실제 내용과 내부 gap만큼만 차지하되, **각 카드 내부에서는 label / 값 / 등락률 열의 세로 기준선을 행 전체가 공유**한다. 시장 카드는 지수값과 등락률도 각각 독립된 열로 정렬하며, Phone은 같은 열 정렬 규칙을 유지한 채 dialog 폭 안에서 중앙 정렬과 overflow 방어만 추가한다.
