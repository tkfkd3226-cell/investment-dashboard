...
- Market AI의 시장/신호 두 카드는 Web·Tablet·Phone 모두 `13:7` 같은 고정 비율을 사용하지 않는다. 공통 `content-driven` 폭을 사용해 각 카드가 실제 label/value 길이와 내부 gap만큼만 차지하고, Phone은 같은 규칙을 유지한 채 dialog 폭 안에서 중앙 정렬과 overflow 방어만 추가한다.
