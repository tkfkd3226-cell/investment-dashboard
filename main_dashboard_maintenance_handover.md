...
- 차트 entrance animation은 card별 최초 viewport 진입 1회가 contract다. `hidden` 자산 panel 또는 `getBoundingClientRect()` 기준 width/height가 0인 chart-wrap은 절대 `chartEntrancePlayed=true`로 승격하지 않는다. 따라서 초기 비활성인 퇴직연금 panel도 실제 탭 전환 후 최초 viewport 진입에서 animation을 재생해야 한다. Live Valuation의 10초 `#app` 갱신에서도 **이미 `chartEntrancePlayed=true`인 card ID만** 완료 상태를 다음 render에 승계하고, 아직 스크롤 진입 전인 card는 pending 상태를 유지한다. 전체 차트를 일괄 완료 처리해 아래쪽 차트의 최초 진입 animation을 소거하는 회귀를 허용하지 않는다.
