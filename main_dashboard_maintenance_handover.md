...
- Web/Tablet에서 개인보기 해제 후 나타나는 투자 계산기와 Market AI 연결 토글은 밝기 테마 버튼과 같은 icon-only control geometry/token을 사용한다. Phone 투자 계산기는 기존 `관리` 메뉴 텍스트 항목을 유지하고 **Market AI 연결 토글은 Phone Topbar에는 두지 않으며 `관리` 메뉴의 투자 계산기 바로 아래 텍스트 action으로 둔다.**
...
- 차트 entrance animation은 card별 최초 viewport 진입 1회가 contract다. Live Valuation의 10초 `#app` 갱신에서도 **이미 `chartEntrancePlayed=true`인 card ID만** 완료 상태를 다음 render에 승계하고, 아직 스크롤 진입 전인 card는 pending 상태를 유지한다. 전체 차트를 일괄 완료 처리해 아래쪽 차트의 최초 진입 animation을 소거하는 회귀를 허용하지 않는다.
- Hero 제목행의 `투자 성과` 기준문구는 `heroPerformanceBasisLabel(date)`가 **실제 계산에 적용된 usable quote coverage**를 기준으로 결정한다. 저장 JSON의 `priceBasis:regular_close`는 `X월 X일 정규장 종가 기준`, 장중 JSON은 `장중 HH:MM 기준`으로 표시한다. 요청 종목 일부만 Market AI usable이면 `일부 실시간 반영`, 모두 usable이고 `extended` live가 하나라도 적용되면 `시간외 포함 현재가 기준`, 모두 usable이며 정규장 live가 적용되면 `실시간 현재가 기준`으로 표시한다. **오늘 20:00 이후 Market AI가 개별주식 애프터 종료를 포함한 요청 universe 전체를 `closed + usable`로 제공하면 `X월 X일 애프터 종가 기준`으로 표시**한다. ETF closed + 개별주식 live/extended가 섞여 있어도 모든 요청 종목이 Market AI usable이면 `일부`로 낮추지 않는다. 과거 legacy close처럼 `priceBasis`가 없는 저장값은 호환을 위해 기존 `종가 기준` 문구를 유지한다. `LIVE / CLOSED / STALE / JSON` 같은 raw 상태 문자열은 노출하지 않는다. 종목·상품명 **라벨 셀 전체 hover** 및 라벨 focus의 기존 `.dash-tooltip`에서 Market AI/JSON 출처·관측시각/기준일을 확인한다. source label은 `aria-describedby`로 기존 tooltip surface와 연결하고 tooltip DOM은 event binding 시 미리 확보한다. 새 tooltip CSS primitive를 만들지 않는다.
