...
- Market AI server reachability는 `dashboard-market-ai.js`가 공통 connection event로 publish하고 Topbar/UI가 소비한다. 서버 연결이 검증된 동안에만 웹/태블릿 Topbar의 `실시간 시세` 버튼과 Phone `관리` 메뉴의 동일 명칭 항목을 노출한다. 두 진입점은 `REALTIME_QUOTES_ACTION` 한 토큰을 공유하고 `dashboard-market-ai-client.js`의 `MARKET_AI_MONITOR_URL`을 사용한다. modal은 고정 px frame을 두지 않고 공통 `--modal-embedded-viewport-width` / `--modal-embedded-viewport-height` token으로 overlay padding을 제외한 현재 viewport 가용 영역을 사용한다. `100dvh` 지원 환경에서는 동적 viewport 높이를 우선하고, modal shell과 iframe은 `overflow:hidden` / `min-height:0` 계약으로 바깥 modal scrollbar를 만들지 않는다. 연결 해제 시 진입점을 숨기고 열린 embedded monitor를 닫아 stale UI를 남기지 않는다.
