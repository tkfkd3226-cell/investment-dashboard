...
- 일반 Print에서는 Topbar·목차·modal·tooltip·toast·보기 전환·차트 조작 UI·Market AI를 제외한다. 단, modal 또는 확대 차트가 열린 상태(`body.dashboard-dialog-open` / `body.chart-expanded-open`)에서는 body 직속 action modal뿐 아니라 `#app` 내부의 퇴직연금 금액 조정 modal도 배경 dashboard와 분리해 현재 surface만 page flow로 출력한다. 확대 차트는 화면용 absolute/full-viewport geometry를 해제하고 stage/chart host 높이를 인쇄용으로 확보해 현재 확대 차트만 한 페이지에 출력한다. 이때 월간 손익의 제목/요일 grid, 히트맵 날짜 select, 실시간 시세 iframe 높이는 화면용 compact 규칙이 인쇄 폭에 새지 않도록 print에서 필요한 축만 복원한다. 닫기 버튼 등 화면 조작 UI는 숨기며, Market AI가 Hero에 mount된 일반 Print에서는 예약 폭을 반환해 Hero 요약 pill이 한 행에 유지되어야 한다.
