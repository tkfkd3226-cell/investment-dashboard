...
Asset Detail은 각 자산 모듈이 계산한 neutral View Model을 받아 **표현만** 담당한다. 증권의 거래·현금/dayChange 계산과 퇴직연금의 납입·상품·현금/dayChange 계산을 common layer로 합치지 않는다. `dashboard-core.js`는 계속 DOM-free를 유지하고, `dashboard-ui-common.js`가 기능 모듈을 역으로 import하지 않는다. 증권 `보유종목 현황`과 퇴직연금 `연금상품별 현황`, 양쪽의 `전일 대비 변동`, `오늘 상승분 기여도`는 같은 Asset UI renderer/CSS를 사용한다. 양쪽 `전일 대비 변동` table view는 401px 이상에서 `자산명 / 전일 종가 / 선택일 종가 / 일변동` 4열을 유지하고, **400px 이하에서는 독립 `일변동` 열을 숨긴 뒤 같은 값을 `선택일 종가` 셀 아래 보조행으로 배치**한다. card view의 정보 구조는 바꾸지 않는다. 두 현황 표의 8개 의미 컬럼은 공통 `asset-status-col-*` semantic class를 유지하되 **컬럼별 고정 px/% 폭이나 `table-layout:fixed`를 강제하지 않고 공통 table의 auto layout과 실제 내용 길이에 따라 자연스럽게 배치**한다. 증권/연금의 실제 렌더 컬럼 폭이 서로 몇 px 달라지는 것은 허용하며, 모바일·1281px 직전에서 가로스크롤을 만들면서까지 폭을 맞추지 않는다. 일반 자산명 강조는 `.asset-item-row` 공통 CSS로 처리한다. 모바일 표의 긴 상품명은 공통 `mobileTableAssetName()` + `.mobile-table-brand-prefix` 표현 규칙을 사용하여 퇴직연금의 `KODEX`, 증권계좌의 `KODEX`·`KOACT/KoAct` 선두 브랜드 prefix만 숨기고 웹·태블릿에서는 원문을 유지한다. 현황 하단의 재투자 기준 안내문은 공통 `.asset-status-basis-note` hook과 기존 `section-explainer`를 함께 사용한다. 증권 `보유종목 현황`의 summary row 구성은 증권 View Model이 소유하며 `보유종목 합계 → 증권계좌 현금 → 총합계`의 3단 구조를 고유 기준으로 계속 유지한다. 증권 `보유종목 현황`·`전일 대비 변동`과 퇴직연금 `연금상품별 현황`·`전일 대비 변동`의 상품 행은 모두 **선택일 평가금액 내림차순**으로 동일하게 정렬하고, 현금성자산·현금·합계 같은 비상품 행은 기존 고정 위치를 유지한다. 증권/연금은 이 정렬을 공통 evaluation sort helper로 재사용하며 현황과 변동에 별도 정렬 기준을 만들지 않는다. 다만 증권계좌 현금은 사용자가 계좌 총액을 맞추기 위해 JSON에 고정하는 장부 보정값이므로 **증권 `전일 대비 변동`의 표/카드/KPI 합계와 `오늘 상승분 기여도`에서는 제외**하고 보유종목의 시장 변동만 사용한다. 이 증권 전일변동 합계는 `누적손익 및 누적수익률`의 전일 대비 손익과 같은 성과 개념을 따라야 한다. 또한 `누적손익 및 누적수익률`의 누적수익률 카드에 표시하는 `전일 대비` 보조 비율은 누적수익률 자체의 %p 증감이 아니라 증권 `전일 대비 변동`의 `하루 변동률`을 그대로 사용하여 `%`로 표시한다. 반대로 퇴직연금 현금성자산은 이자·배당 등이 실제 자산 증가로 반영되는 운용자산이므로 기존처럼 전일변동과 기여도에 포함하며, `운용손익 및 운용수익률`의 운용수익률 카드 역시 퇴직연금 `하루 변동률`을 동일한 `%` 기준으로 표시한다. 누적/운용 수익률의 본값 계산은 이 보조표시 변경으로 바꾸지 않는다. 상승분 기여도는 각 자산 adapter가 계산한 양수 dayChange 항목만 common renderer에 전달한다. 증권/연금 tooltip ID는 각 asset prefix로 고유하게 만들고, tooltip interaction은 app에서 공통 `.asset-insight-zone`을 대상으로 한 번 초기화한다.

증권과 퇴직연금의 상단 KPI는 공통 **`성과 요약` overview shell**을 사용한다. 기본 `전체` 상태에서 제목 시작 위치, KPI 시작 위치·높이·간격, KPI 하단 여백, 다음 Asset Detail 시작 위치가 양쪽 자산에서 같은 vertical rhythm을 가져야 한다. 증권만 `성과 요약` header action으로 `전체 / 계좌별` drill-down을 제공하며 기본값은 `전체`다. `계좌별`은 과거 독립 `계좌별 성과 요약` 섹션을 별도로 유지하지 않고 같은 overview 안에서 table/card view로 전환한다. 계좌1·계좌2·토스·합계를 유지하며, 계좌별 **table 열 순서는 `구분 → 투자 결과물 → 투입원금 → 누적손익 → 누적수익률 → 메모`**로 두고, 모바일 계좌별 카드도 `투자 결과물 → 투입원금` 순서와 같은 용어를 사용하여 전체 성과 카드와 맞춘다. 계좌별 `투입원금`과 `투자 결과물`은 **각 계좌 행에서 윗줄에 성과 기준값(A), 아랫줄에 `조정 B`를 2단으로 표시**한다. 각 계좌의 A값은 `투자 결과물 A - 투입원금 A = 누적손익`이 성립해야 하며, A에 B를 반영한 값은 기존 전체 성과 요약의 장부 기준 투입원금/투자 결과물과 검산되어야 한다. **합계 행의 `투자 결과물`과 `투입원금`은 별도의 합계 조정 B를 다시 표시하지 않고, 각 계좌의 `A + 조정 B` 값을 합산한 최종 장부 기준값으로 직접 표시**하여 `전체` 카드의 투자 결과물/투입원금과 일치시킨다. **별도수익 ON/OFF는 기존 `separateProfitView()`와 재분류 금액을 그대로 따라 A의 투입원금·투자 결과물·누적손익을 함께 변경**하고, B는 계좌 간 이동·중복 제거라는 장부 목적을 유지하여 별도수익 스위치 때문에 임의로 바꾸지 않는다. 보유 자금 투입이 존재할 때 **개인 기능이 비활성인 상태에서는 `별도수익`, `ON/OFF`처럼 개인 기능의 존재를 직접 드러내는 표현은 쓰지 않되, 해당 `보유 자금 투입`을 성과기준 원금에 포함했다는 장부 설명은 계좌별 메모에 유지한다.** 별도수익이 실제로 활성화된 상태에서는 삼성증권1 메모에 누적 별도수익 중 재투입분이 성과기준 원금에서 제외되는 이유를 설명한다. B는 새 계산체계를 만들지 않고 기존 계좌1 투자원금 검산, 계좌2·토스의 삼성증권1 재투입액, **실현수익 투입** 등 기존 장부 값을 그대로 근거로 사용하며, 메모에서도 `레버수익 재투입`, `VIP 수익 재투입`, `실현수익 투입`, `원천·보유 차액`, `보유 자금 투입`처럼 **계좌1 투자원금 검산에서 실제 사용하는 용어를 그대로 재사용**하되, `원천·보유 차액`은 원천과 실제 보유원가 사이의 중립 검산값으로 성과기준 투입원금에는 남기고 계좌 간 재투입·중복 제거 조정 B의 근거에서는 제외한다. 계좌1 투입원금 조정값은 해당 선택일의 `레버수익 재투입 + VIP 수익 재투입 + 실현수익 투입` 합계를 중복 제거 근거로 사용하며, 과거 장부 구간 등 두 값이 정확히 일치하지 않는 경우에는 `전체 투입원금과 계좌1 투자원금 간 장부 차이를 반영한 조정값`처럼 중립적으로 설명한다. 계좌1 투입원금 조정값 자체는 계좌별 성과 표의 다른 조정값과 동일한 중립 스타일을 유지한다. 다만 해당 선택일에서 조정값이 `레버수익 재투입 + VIP 수익 재투입 + 실현수익 투입`과 정확히 검산되어 상세 근거 문구를 표시하는 경우에는, 메모 중 `조정금액원은 계좌1 투자원금 검산의 레버수익 재투입·VIP 수익 재투입·실현수익 투입` 구간만 검산표의 derived 배경과 같은 색으로 연결하고 같은 caution 계열 테두리를 더해 연결 표시가 눈에 보이도록 한다. 삼성증권2의 투자 결과물 조정은 **VIP 재투입액**의 중복 제거이며, 메모에서 `VIP 금 투입분 + VIP 재투입-금 = VIP 재투입액`의 원천 조합을 함께 명시한다. 계좌별 합계의 누적수익률은 `전체` 카드와 동일한 장부 기준을 사용하여 `separateProfitView()`의 전체 누적수익률과 정확히 일치해야 한다. 즉 합계 행의 투입원금은 각 계좌의 `A + 조정 B` 합계이며, 이 조정 반영 후 전체 보기의 투입원금을 분모로 사용한다. `전체` 성과 카드의 누적손익 설명은 **장부 검산 기준일인 2026-06-18 이후에만 `투자 결과물 - 투입원금`을 표시**하고, 그 이전 복원 구간은 과거 장부의 재투입·실현 구조가 달라 단순 차감식이 성립하지 않으므로 `전체 누적 성과 기준`처럼 중립적으로 표시한다. 과거 수치 자체를 이 설명에 맞추기 위해 재계산하지 않는다. `투자원금 원천 및 검산`의 계좌1 투자원금 검산 표에서는 `전체 투입원금` 바로 아래에 `보유 자금 투입`을 배치하고 두 base 행은 별도 파란 강조 없이 카드 기본 배경(라이트에서는 흰색)을 사용한다. 그 아래 `레버수익 재투입 / VIP 수익 재투입 / 실현수익 투입`은 서로 같은 매우 연한 derived 배경 그룹으로 묶고, `원천·보유 차액`은 조정 B와 구분되는 중립 배경을 유지하며, `계좌1 원천별 추적`은 3단 카드 구성을 유지한 채 투자원금 검산의 항목 순서와 원천 관계를 따라 배치한다. `전체 투입원금`에 대응하는 `금 판매액 투입 / VIP 금 투입분 / 근로소득 투입액`, `보유 자금 투입`, `레버수익 재투입`, `VIP 수익 재투입`에 대응하는 `VIP 재투입-금 / 임시자금 투입 / 원금 회수`, `실현수익 투입`, `원천·보유 차액` 순으로 표시하며, 각 원천 행의 배경은 대응되는 검산 행의 base/derived 배경과 동일하게 맞추되 `원천·보유 차액`은 양쪽 표 모두 중립 배경을 사용한다. 계좌별 표는 웹·태블릿에서 누적손익/누적수익률을 분리한 6열을 유지하고, **모바일(≤760px 및 실제 스마트폰 가로모드)에서만 누적수익률을 누적손익 셀의 다음 줄에 괄호형 보조값으로 배치하여 5열로 축약**한다. 모바일에서도 손익과 수익률의 가독성 차이를 과도하게 만들기 위해 수익률 글자를 추가 축소하지 않는다. 400px 이하에서도 메모 헤더 문구는 `메모`를 유지하고, 각 행의 메모 내용만 기존 작은 원형 정보 버튼으로 전환하며 계좌별 표의 좌우 cell padding은 compact 값을 사용한다. 401px 이상에서는 계좌별 table이 wrapper 폭을 우선 사용한다. 숫자열은 `nowrap`을 유지하고, **웹·태블릿(761px 이상)에서는 메모 셀만 가용폭이 부족해지는 시점부터 자연스럽게 줄바꿈**한다. 모바일 401~760px 및 실제 스마트폰 가로모드의 **표 보기에서만** `.`으로 끝나는 문장 단위 줄바꿈은 유지하되, 각 문장 내부는 `nowrap`으로 유지하여 한 문장이 중간에서 추가 줄바꿈되지 않게 한다. **카드 보기에서는 같은 문장 block을 유지하되 `nowrap`을 적용하지 않고 카드 폭에 맞춰 자연스럽게 줄바꿈**한다. 삼성증권2의 `2023-12-20 최초 시작. 2026-05-22 전량 매도.` 두 이력 문장은 하나의 history block으로 묶어 강제 문장 분리 없이 같은 줄 흐름으로 표시한다. 계좌별 view가 높아지는 것은 자연스러운 content flow로 처리하고, 전체 복귀 시 고정 min-height나 placeholder 없이 기본 높이로 복원한다. 이 overview의 title/action은 기존 `.section-title`, `.section-title-icon`, segmented/button, mobile table/card control을 우선 재사용하며 별도 전용 디자인 시스템을 만들지 않는다. 증권과 퇴직연금의 `성과 요약` 제목은 같은 기존 `chart` 아이콘과 동일한 section-title 표현을 사용하고, 목차의 자산 진입 항목도 실제 메인 `성과 요약`과 같은 기존 `chart` 아이콘을 재사용하며 `증권계좌 성과 요약` / `퇴직연금 성과 요약`으로 표시한다. 계좌별 합계 메모는 `각 계좌의 투자 결과물·투입원금에 계좌 간 재투입 및 중복 제거 조정을 반영한 최종 합계.`를 기준 문구로 사용한다. 증권 `전체 / 계좌별` segmented switch의 두 버튼은 같은 폭을 유지하고, 데스크톱에서는 `계좌별` 전용 표/카드 보기 wrapper가 빈 flex item이나 gap을 만들지 않아 `전체`/`계좌별` 어느 상태에서도 segmented switch의 우측 끝 위치와 여백이 정확히 같아야 한다. **증권계좌 현황 제목 우측에 두던 `별도수익 ON/OFF`는 성과 요약 header의 `전체 / 계좌별` segmented switch 바로 왼쪽으로 배치**하며, `누적손익 및 누적수익률` chart header의 `별도수익 ON/OFF → 수익률/코스피` 배치처럼 별도수익 control이 먼저 오고 segmented switch가 뒤따르는 간격·정렬을 재사용한다. 모바일에서 전용 표/카드 보기 버튼이 나타날 때 액션 순서는 **`표/카드 보기 → 별도수익 ON/OFF → 전체/계좌별 segmented switch`**로 두며, 기존 `.chart-head-actions`의 공통 gap/정렬을 그대로 사용한다.
...
`증권계좌 현황` 바로 아래 4개 KPI 카드와 `퇴직연금 현황` 바로 아래 4개 KPI 카드는 모바일(`<=760px` 및 실제 스마트폰 가로모드)에서만 `2 × 2` grid를 유지한다. 다른 `.metric-grid`에는 이 규칙을 확대 적용하지 않는다.
...
현재 실제 구현에서는 **400px 이하**에서도 계좌별 성과 메모 헤더 문구는 `메모`를 유지하고, 각 행의 메모 내용만 작은 원형 정보 버튼으로 전환한다.
...
- 최대 폭은 `calc(100vw - 28px)`이며 화면을 넘어갈 때만 줄바꿈한다.
- 글자색은 기존 공통 token `--tooltip-color`를 사용한다.
- 좌우 여백을 보정하고 아래 공간이 부족하면 버튼 위로 flip한다.
- outside click / change / ESC / scroll / resize에서 열린 상태와 floating DOM을 정리한다.
- 401px 이상에서는 기존 메모 텍스트 표시 구조를 유지한다. 여러 문장으로 구성된 메모는 **웹·태블릿(761px 이상)에서는 `.`으로 끝나는 각 문장을 새 줄로 분리**하고 메모 셀 자체도 필요 시 자연스럽게 줄바꿈한다. **모바일 401~760px 및 실제 스마트폰 가로모드의 표 보기에서도 문장별 줄바꿈은 유지**하되, 각 문장 내부는 `nowrap`으로 유지하여 한 문장이 중간에서 추가 줄바꿈되지 않게 한다. **모바일 카드 보기에서는 문장 block을 유지하면서 문장 내부는 카드 폭에 맞춰 자연스럽게 줄바꿈**한다. **400px 이하에서는 기존 정보 버튼 전환을 그대로 유지**한다.

이 기능을 수정할 때 계좌 메모 전용 동작을 `dashboard-charts.js`로 다시 옮기지 않는다.
