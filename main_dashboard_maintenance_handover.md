...
- 단건 저장은 frontend가 같은 요청의 실패·재시도 동안 identity를 재사용한다. 기업적립금·ETF 추가매수는 client-generated ID를, 현금성자산은 `requestId`를 사용한다. GAS는 resource 내부 identity뿐 아니라 최근 완료 receipt와 진행 intent를 별도 보존하며, GitHub write 예외가 발생해도 mutation 성공 여부가 불확실한 intent를 지우지 않는다. 따라서 최초 저장 뒤 응답이 유실되거나 write 결과가 불확실한 상태에서 이후 같은 resource가 갱신/삭제되어도 과거 save retry가 최신 상태를 되돌리지 않는다. 같은 identity에 다른 내용이 오면 거부하며, stale retry는 `stale_retry_ignored` + `duplicate:true`로 mutation 없이 수렴한다.
...
- 단건 Pension 저장/삭제와 Batch는 같은 `ScriptLock` mutation boundary를 사용한다. contribution/ETF 삭제가 연결 snapshot을 확인하는 동안 다른 단건 cashSnapshot 저장이 끼어 참조 관계를 깨뜨리는 cross-file race를 허용하지 않는다. Batch는 frontend의 `batchRequestId`와 각 작업의 stable `operationId`를 함께 전달하고, GAS는 commit 전에 `baseCommitSha`를 포함한 intent를 기록하며 contribution/ETF는 이 identity에서 결정적 저장 ID를 생성한다. receipt 저장이 실패해도 재시도 시 현재 GitHub state가 해당 batch 결과를 이미 반영했는지 먼저 확인한다. base commit 이후 최신 update/delete가 생겨 batch 결과가 더 이상 현재 상태와 일치하지 않으면 `batch_stale_retry_ignored`로 종료한다. Cache에는 과거 전체 pension state를 저장하지 않고 receipt형 요약만 저장한다.
- KRX 갱신은 frontend가 같은 실패·재시도 동안 `requestId`를 재사용하고 GAS가 dispatch 전에 request별 intent를 durable하게 저장한 뒤 성공 receipt로 승격한다. receipt 저장/응답이 유실되어 intent만 남으면 동일 requestId는 `workflow_dispatch_uncertain`으로 중복 dispatch하지 않는다. `.github/workflows/update-prices.yml`은 같은 branch를 `concurrency + queue: max`로 직렬화해 pending 실행도 보존하고, push 직전 remote를 다시 fetch한다. checkout 이후 다른 commit이 branch를 앞서가면 KRX 관리파일뿐 아니라 generation input(`portfolio.json`, updater, requirements, workflow) 변경 여부도 확인한다. 계산과 무관한 commit만 최신 remote에 rebase하여 최대 3회 push 재시도하며, managed file 또는 generation input이 바뀌었으면 과거 계산 결과를 올리지 않고 fail-closed한다.
...
- Batch retry QA에서는 GitHub commit 성공 직후 receipt/HTTP 응답이 실패한 상태뿐 아니라 `batch A → 최신 update/delete B → A 재시도` 순서도 만들고, A가 B를 되돌리지 않는지 확인한다. Pension/KRX receipt·intent direct property는 request별 compact property를 사용하고 180일 + prefix별 최대 건수 GC를 적용해 단일 9KB뿐 아니라 Script Properties 전체 저장량도 무한 증가하지 않는지 확인한다.
- KRX retry QA에서는 dispatch 성공 뒤 receipt/응답 유실을 만들고 동일 `requestId`가 intent 때문에 재-dispatch되지 않는지 확인한다. 또한 KRX checkout 뒤 pension 저장 같은 무관 commit은 rebase 가능한지, `prices.json`/`performance_snapshots.json` 또는 `portfolio.json`/updater/requirements/workflow가 바뀐 경우에는 fail-closed하는지 확인한다. concurrency QA에서는 A 실행 중 B·C가 연속 dispatch되어도 `queue: max`로 B가 pending 교체 취소되지 않는지 contract를 확인한다.
