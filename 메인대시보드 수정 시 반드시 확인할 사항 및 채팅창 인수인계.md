...
최신 전체 ZIP이 첨부된 상태에서 GPT가 `인수인계` 요청을 받으면:
...
## C. GitHub ZIP과 GAS의 범위

Google Apps Script는 GitHub ZIP 구성요소가 아니다.
...
가 없다는 이유로 구조 누락이나 감점으로 처리하지 않는다.
...
→ GAS server handler 내부는 "최신 Code.gs 미제공으로 직접 검증 불가"라고만 표시
→ 이것 자체로 CSS / JS / UI / UX 점수를 감점하지 않음

최신 운영 Code.gs도 별도 제공
...
> **Google Apps Script는 위 GitHub 저장소 구조에 포함되지 않는다.**  
> GAS는 Google Apps Script 프로젝트에서 단일 `Code.gs`로 별도 관리·별도 배포하며, 최신 GitHub ZIP에 `gas/Code.gs`가 없는 것은 정상이다.
...
# 14. 메인 JavaScript는 현재 7파일 ES Module 구조 유지
...
# 18. `dashboard-ui.js`와 `dashboard-ui-common.js` 책임
...
# 63. 2026-08-18 JS 3차 리팩토링 최종 QA PASS 기준선
...
# 100. JS 구조 리팩토링 차수 작업 · QA · 보고 운영 원칙
...
# 101. Google Apps Script(GAS) 별도 관리 및 배포 원칙

Google Apps Script는 **GitHub 저장소와 별개의 운영 소스**다.

기본 원칙:

```text
GitHub 저장소
→ index.html / css / js / data / Python / GitHub Actions 관리
...
- 최신 GitHub ZIP에 `gas/Code.gs` 또는 `gas/` 폴더가 없어도 **누락이나 구조 오류로 판단하지 않는다.**
- GAS 소스의 source of truth는 **현재 Google Apps Script 프로젝트에서 실제 운영 중인 최신 `Code.gs`**다.
- GAS를 수정할 때 과거 대화에서 기억한 코드나 GitHub ZIP을 최신 GAS라고 추정하지 않는다.
- GAS 수정이 필요한 경우 사용자가 제공한 최신 운영 `Code.gs` 전체를 직접 읽고 그 파일만 기준으로 수정한다.
- GAS는 사용자 운영 원칙상 **여러 `.gs` 파일로 분리하지 않고 단일 `Code.gs`를 유지**한다.
- 기능별 구조화가 필요하면 단일 파일 내부에서 section 주석, 함수 배치, helper 공통화, business/Web layer 분리로 정리한다.

운영 Script Properties:
...
실제 값은 GitHub 저장소나 README/MD에 기록하지 않는다.

배포 흐름:

```text
Google Apps Script 단일 Code.gs
→ Script Properties 사용
→ Web App으로 배포
→ /exec URL을 메인 dashboard JS가 호출
→ GitHub REST API 또는 GitHub Actions workflow_dispatch 실행
```

`Code.gs`를 수정했다고 해서 운영 Web App에 자동 반영된다고 가정하지 않는다.

- 코드 수정 후에는 **운영 Web App 배포를 새 코드 버전으로 갱신**한다.
- 가능하면 기존 운영 배포를 갱신하여 기존 `/exec` URL을 유지한다.
- 새 Web App 배포 URL을 만들었다면 메인 JS의 GAS API URL도 새 URL로 맞춰야 한다.
- GAS QA에서는 실제 운영 JSON write, delete, batch apply, KRX workflow 실행을 하지 않고 mock/stub을 우선 사용한다.

JS ↔ GAS 계약 검증 시:

- 프런트 JS의 요청 payload는 GitHub ZIP에서 확인할 수 있다.
- 서버 측 handler 검증은 **최신 운영 `Code.gs`가 별도로 제공된 경우에만 완전 검증**한다.
- GitHub ZIP만 보고 `Code.gs`가 없다는 이유로 JS↔GAS 계약을 FAIL 처리하지 않는다.
