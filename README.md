...
- 모바일 카드/표 보기 전환
- 퇴직연금 현금성자산·기업적립금·추가 매수 등록·삭제
- KRX 현재가 최신/누락 반영 및 선택일 재갱신
- 누적수익률과 코스피 지수 차트 전환
- 웹 우측 가장자리 마우스오버 목차 및 모바일 햄버거 통합 메뉴
- 모바일 우측 하단 `TOP` 버튼
- 별도 투자 계산기(`calc.html`)
...
- 체결수량과 체결금액을 기준으로 현금성자산에서 차감합니다.
...
- 매수원금은 현금성자산의 평가이익을 먼저 사용한 뒤 부족한 금액만 차감합니다. 따라서 평가이익이 있는 경우 `체결금액 = 매수원금 감소액`이 아닐 수 있습니다.
...
```text
최근 현금성자산 평가금액 스냅샷 + 스냅샷 이후 추가 기업적립금 - 이후 추가 매수 금액
```
