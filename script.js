...
function prepareChartEntranceForSvg(svg){
  ...
  chartEntranceDataElements(svg).forEach(node=>{
    ...
    if(tag==='polyline'){
      ...
      node.style.setProperty('--chart-delay',`${53+lineIndex*37}ms`);
      lineIndex+=1;
      return;
    }
    const delay=Math.round(chartEntranceXRatio(node,svg)*453);
...
function pensionRow(r,total){const w=total?r.evalAmount/total*100:0;return `<tr><td><strong>${mobileTableProductName(r.name)}</strong>${pensionProductSwatch(r.name)}</td><td class="num">${fmt(r.qty)}</td><td class="num">${fmt(r.qty?r.cost/r.qty:0)}</td><td class="num">${fmt(r.cost)}</td><td class="num">${fmt(r.evalAmount)}</td><td class="num ${cls(r.profit)}">${fmt(r.profit)}</td><td class="num ${cls(r.returnRate)}">${pct(r.returnRate)}</td><td><div class="bar-box"><div class="bar-fill ${r.barClass}" style="width:${Math.max(0,Math.min(100,w)).toFixed(1)}%;background:${pensionSeriesColor(r.name)}"></div></div><div class="small">${w.toFixed(1)}%</div></td></tr>`}
...
function renderSourceTables(x){
  const c=PORTFOLIO.constants,
    ...
    reconciliationRow=`<tr><td>원천·보유 차액</td><td class="num ${cls(sourceHoldingGap)}">${signed(sourceHoldingGap)}</td></tr>`,
  ...
  return `<section id="capital-source-check" class="capital-source-section"><div class="section-title source-title"><h2><span class="section-title-icon">🧾</span>투자원금 원천 및 검산</h2>${separateProfitControl(x,'section-inline')}</div><div class="grid three source-grid"><div class="card source-card"><div class="label">계좌1 원천별 추적</div><div class="value">${won(performancePrincipal)}</div><table style="font-size:12px;margin-top:12px;border-radius:12px"><tbody><tr><td>금 판매액 투입</td><td class="num">4,000,000</td></tr><tr><td>근로소득 투입</td><td class="num">7,036,104</td></tr><tr><td>임시자금 투입</td><td class="num">4,955,580</td></tr><tr><td>원금 회수</td><td class="num negative">-6,089,845</td></tr><tr><td>레버수익 재투입</td><td class="num">${fmt(c.tossReinvestedToAccount1)}</td></tr><tr><td>VIP 재투입</td><td class="num">${fmt(c.account2ReinvestedToAccount1)}</td></tr>${excludedRow}${internalCashRow}${extraRow}${reconciliationRow}<tr class="summary-row"><td>합계</td><td class="num">${fmt(performancePrincipal)}</td></tr></tbody></table></div><div class="card source-card highlight"><div class="label">전체 투입원금</div><div class="value">${won(externalPrincipal)}</div><table style="font-size:12px;margin-top:12px;border-radius:12px"><tbody><tr><td>금 판매액 총액</td><td class="num">${fmt(c.goldPrincipal)}</td></tr><tr><td>근로소득 투입액</td><td class="num">${fmt(c.laborNetPrincipal)}</td></tr>${extraRow}<tr class="summary-row"><td>합계</td><td class="num">${fmt(externalPrincipal)}</td></tr></tbody></table>${reclassNote}</div><div class="card source-card"><div class="label">${principalLabel}</div><div class="value">${won(performancePrincipal)}</div><table style="font-size:12px;margin-top:12px;border-radius:12px"><tbody><tr><td>전체 투입원금</td><td class="num">${fmt(externalPrincipal)}</td></tr><tr><td>레버수익 재투입</td><td class="num">${fmt(c.tossReinvestedToAccount1)}</td></tr><tr><td>VIP 수익 재투입</td><td class="num">${fmt(vipProfitReinvest)}</td></tr>${excludedRow}${internalCashRow}${reconciliationRow}<tr class="summary-row"><td>${principalSummaryLabel}</td><td class="num">${fmt(performancePrincipal)}</td></tr></tbody></table></div></div></section>`;
}
