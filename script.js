...
const securityAllocationColor=name=>{
  ...
  const idx=Math.max(0,(PORTFOLIO?.securities||[]).findIndex(r=>r.name===name));
...
const account1SourcePrincipalForDate=d=>(Number(PORTFOLIO?.constants?.account1Principal)||0)-securityContributionAfter(d)+securityWithdrawalAfter(d);
const securitiesHoldingCostForDate=d=>{
  const daily=ACCOUNT1_DAILY?.[d];
  if(Array.isArray(daily?.holdings)) return daily.holdings.reduce((a,h)=>a+(Number(h?.cost)||0),0);
  return (PORTFOLIO?.securities||[]).reduce((a,h)=>a+(Number(securityPositionState(h,d)?.cost)||0),0);
};
...
function chartDisplayLabel(scope,label){
  ...
  const symbolScope=scope==='pensionSymbol'||scope==='pensionAlloc'||scope==='securitiesSymbol'||(scope==='securitiesAlloc'&&SECURITY_ALLOC_MODE==='symbol');
...
function chartStateKey(scope){
  return scope==='securitiesAlloc'?`securitiesAlloc:${SECURITY_ALLOC_MODE==='symbol'?'symbol':'type'}`:scope;
}
...
function chartLegendItems(scope){
  if(scope==='securitiesCum')return [
    ...
    {key:'compare',label:chartCompareLabel('securities'),color:CHART_COMPARE_MODES.securities==='kospi'?'#7c3aed':'#5abdf2'}
  ];
  if(scope==='pensionCum')return [
    {key:'profit',label:'운용수익',color:'#ffb84d'},
    {key:'daily',label:'전일대비손익',color:'#a7d7a8'},
    {key:'compare',label:chartCompareLabel('pension'),color:CHART_COMPARE_MODES.pension==='kospi'?'#7c3aed':'#5abdf2'}
  ];
  if(scope==='securitiesSymbol'){
    const current=ACTIVE_DATE?calc(ACTIVE_DATE):null;
    const activeNames=new Set(ACTIVE_DATE?securityChartNamesForDate(ACTIVE_DATE):[]);
...
function securityAllocCardCount(x){
  return SECURITY_ALLOC_MODE==='symbol'?4:3;
}
function securityAllocCardsHtml(x){
  ...
  const totalCard=`<div class="mini-card alloc-total-card${SECURITY_ALLOC_MODE==='symbol'?' alloc-total-card-wide':''}"><div class="m-label">현재 평가총액</div><div class="m-value">${won(x.allocTotal)}</div>${includeDetail}</div>`;
  if(SECURITY_ALLOC_MODE!=='symbol'){
...
function setSecurityAllocMode(mode){
  ...
  document.querySelectorAll('[data-security-alloc-mode]').forEach(btn=>{
    const active=btn.dataset.securityAllocMode===SECURITY_ALLOC_MODE;
    btn.classList.toggle('active',active);
    btn.setAttribute('aria-pressed',String(active));
  });
  const x=ACTIVE_DATE?calc(ACTIVE_DATE):null;
...
function pensionRow(r,total){const w=total?r.evalAmount/total*100:0;return `<tr><td><strong>${mobileTableProductName(r.name)}</strong>${pensionProductSwatch(r.name)}</td><td class="num table-cell-center">${fmt(r.qty)}</td><td class="num">${fmt(r.qty?r.cost/r.qty:0)}</td><td class="num">${fmt(r.cost)}</td><td class="num">${fmt(r.evalAmount)}</td><td class="num ${tableCls(r.profit)}">${fmt(r.profit)}</td><td class="num table-cell-center ${tableCls(r.returnRate)}">${pct(r.returnRate)}</td><td><div class="bar-box"><div class="bar-fill ${r.barClass}" style="width:${Math.max(0,Math.min(100,w)).toFixed(1)}%;background:${pensionSeriesColor(r.name)}"></div></div><div class="small">${w.toFixed(1)}%</div></td></tr>`}
...
function renderCharts(x){
  ...
  <div class="chart-card" id="chart-symbol"><div class="chart-head"><div><h3><span class="section-title-icon chart-icon">🧩</span>종목별 누적손익</h3></div><div class="chart-head-actions">${symbolChartToggle('securities')}${chartWebExpandButton()}</div></div>${chartScrollButton()}<div class="chart-wrap"><svg class="chart" id="chartSymbol"></svg></div><div class="chart-legend" id="securitiesSymbolLegend">${chartLegendHtml('securitiesSymbol')}</div><div class="chart-note symbol-summary-grid">${orderedSymbols.map(h=>symbolCard(h,symbolTotal)).join('')}</div><div class="symbol-summary-note">${hasSymbolBuyFlow?'ㆍ기여도: 누적손익 합계 기준&nbsp;&nbsp;ㆍ손익률: 누적손익 ÷ 매입원금&nbsp;&nbsp;ㆍ전일대비 변동률: 전일대비 변동액 ÷ (전일 평가액 + 당일 매수금액)':'ㆍ기여도: 누적손익 합계 기준&nbsp;&nbsp;ㆍ손익률: 누적손익 ÷ 매입원금&nbsp;&nbsp;ㆍ전일대비 변동률: 전일대비 변동액 ÷ 전일의 평가금액'}</div></div>
...
function renderPensionCharts(x){
  ...
  const allocCards=orderedAllocRows.map(r=>`<div class="mini-card"><div class="m-label">${r.name}${pensionProductSwatch(r.name)}</div><div class="m-value">${won(r.evalAmount)} <span class="small">(${(r.evalAmount/productEvalTotal*100).toFixed(1)}%)</span></div></div>`).join('');
...
function renderSourceTables(x){
  const c=PORTFOLIO.constants,
    ...
    excludedRow=!INCLUDE_SEPARATE_PROFIT&&excludedTransfer?`<tr><td>보유 자금 투입</td><td class="num">${fmt(excludedTransfer)}</td></tr>`:'',
    internalCashRow=internalCashTransfer?`<tr><td>실현수익 투입</td><td class="num">${fmt(internalCashTransfer)}</td></tr>`:'',
    reconciliationRow=`<tr><td>원천·보유 차액</td><td class="num ${tableCls(sourceHoldingGap)}">${signed(sourceHoldingGap)}</td></tr>`,
    ...
    principalLabel=INCLUDE_SEPARATE_PROFIT&&reclassified?'계좌1 성과기준 투자원금':'계좌1 투자원금 검산',
    principalSummaryLabel='합계';
  return `<section id="capital-source-check" class="capital-source-section"><div class="section-title source-title"><h2><span class="section-title-icon">🧾</span>투자원금 원천 및 검산</h2>${separateProfitControl(x,'section-inline')}</div><div class="grid three source-grid"><div class="card source-card"><div class="label">계좌1 원천별 추적</div><div class="value">${won(performancePrincipal)}</div><div class="mobile-scroll source-table-scroll"><table class="dashboard-data-table source-data-table"><tbody><tr><td>금 판매액 투입</td><td class="num">4,000,000</td></tr><tr><td>근로소득 투입</td><td class="num">7,036,104</td></tr><tr><td>임시자금 투입</td><td class="num">4,955,580</td></tr><tr><td>원금 회수</td><td class="num table-negative">-6,089,845</td></tr><tr><td>레버수익 재투입</td><td class="num">${fmt(c.tossReinvestedToAccount1)}</td></tr><tr><td>VIP 재투입</td><td class="num">${fmt(c.account2ReinvestedToAccount1)}</td></tr>${excludedRow}${internalCashRow}${extraRow}${reconciliationRow}<tr class="summary-row"><td>합계</td><td class="num">${fmt(performancePrincipal)}</td></tr></tbody></table></div></div><div class="card source-card highlight"><div class="label">전체 투입원금</div><div class="value">${won(externalPrincipal)}</div><div class="mobile-scroll source-table-scroll"><table class="dashboard-data-table source-data-table"><tbody><tr><td>금 판매액 총액</td><td class="num">${fmt(c.goldPrincipal)}</td></tr><tr><td>근로소득 투입액</td><td class="num">${fmt(c.laborNetPrincipal)}</td></tr>${extraRow}<tr class="summary-row"><td>합계</td><td class="num">${fmt(externalPrincipal)}</td></tr></tbody></table></div>${reclassNote}</div><div class="card source-card"><div class="label">${principalLabel}</div><div class="value">${won(performancePrincipal)}</div><div class="mobile-scroll source-table-scroll"><table class="dashboard-data-table source-data-table"><tbody><tr><td>전체 투입원금</td><td class="num">${fmt(externalPrincipal)}</td></tr><tr><td>레버수익 재투입</td><td class="num">${fmt(c.tossReinvestedToAccount1)}</td></tr><tr><td>VIP 수익 재투입</td><td class="num">${fmt(vipProfitReinvest)}</td></tr>${excludedRow}${internalCashRow}${reconciliationRow}<tr class="summary-row"><td>${principalSummaryLabel}</td><td class="num">${fmt(performancePrincipal)}</td></tr></tbody></table></div></div></div></section>`;
}
...
function pensionSeriesColor(name){
  ...
  if(name==='KODEX 삼전채권혼합')return '#66bb6a';
...
function drawPensionCumChart(){
  ...
  const mode=CHART_COMPARE_MODES.pension||'return',selection=chartSelection('pensionCum'),selected=selection.selected,autoY=chartAutoYEnabled('pensionCum');
...
function drawPensionSymbolChart(){
  ...
  const mode=SYMBOL_CHART_MODES.pension||'profit',selection=chartSelection('pensionSymbol'),allSeries=chartLegendItems('pensionSymbol').map(item=>item.key),series=allSeries.filter(name=>selection.selected.has(name)),autoY=chartAutoYEnabled('pensionSymbol');
...
function drawCumChart(){
  ...
  const mode=CHART_COMPARE_MODES.securities||'return',selection=chartSelection('securitiesCum'),selected=selection.selected,autoY=chartAutoYEnabled('securitiesCum');
...
function drawLineChart(){
  ...
  const mode=SYMBOL_CHART_MODES.securities||'profit',selection=chartSelection('securitiesSymbol'),allSeries=chartLegendItems('securitiesSymbol').map(item=>item.key),series=allSeries.filter(s=>selection.selected.has(s)),autoY=chartAutoYEnabled('securitiesSymbol');
...
function drawStacked(){
  ...
  const mode=SECURITY_ALLOC_MODE==='symbol'?'symbol':'type',selection=chartSelection('securitiesAlloc'),allSeries=chartLegendItems('securitiesAlloc').map(item=>item.key),series=allSeries.filter(key=>selection.selected.has(key)),autoY=chartAutoYEnabled('securitiesAlloc');
  const symbolSeries=mode==='symbol'?allSeries.filter(key=>key!=='현금'):[];
  const data=mode==='symbol'?securitySymbolAllocHistory(ACTIVE_DATE,symbolSeries):allocHistory(ACTIVE_DATE);
  if(!data.length)return;
  const colors=mode==='symbol'?Object.fromEntries(allSeries.map(name=>[name,name==='현금'?CASH_ASSET_COLOR:securityAllocationColor(name)])):{ETF:'#ff6b6b',개별주식:'#ffc857',현금:CASH_ASSET_COLOR};
