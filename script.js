...
function renderPensionProductInsights(x){
  ...
  const topHtml=x.pensionPrevEval==null
    ...
    : items.length
      ? `<div class="pension-stack-compact-head"><div><div class="pension-stack-kicker">${x.prevKey?`${shortDate(x.prevKey)} → ${shortDate(x.date)}`:'오늘'} 상승 기여 합계</div><div class="pension-stack-total positive">${signed(risingTotal,'원')}</div></div><div class="pension-stack-chip">마우스 오버로 상세</div></div><div class="pension-stack-bar compact">${items.map(item=>`<div class="pension-stack-segment" title="${item.name} / ${item.share.toFixed(1)}% / ${signed(item.value)}" style="width:${Math.max(item.share,2).toFixed(2)}%;background:${item.color}"><span>${item.share>=13?item.name.replace('KODEX ',''):''}</span></div>`).join('')}</div><div class="pension-stack-inline-legend">${items.map(item=>`<span title="${item.name} / ${item.share.toFixed(1)}% / ${signed(item.value)}"><i style="background:${item.color}"></i>${item.name.replace('KODEX ','')} ${item.share.toFixed(1)}%</span>`).join('')}</div>`
      : `<div class="pension-empty-state">상승한 자산이 없어 기여도를 표시하지 않습니다.</div>`;
  return `<div class="pension-insight-zone"><div class="pension-insight-card"><div class="pension-insight-head"><div><div class="pension-insight-eyebrow">TODAY CONTRIBUTION</div><h3>오늘 상승분 기여도</h3></div><span class="pension-insight-badge blue">STACK BAR</span></div>${topHtml}</div><div class="pension-insight-card"><div class="pension-insight-head"><div><div class="pension-insight-eyebrow">RISK ASSET RULE</div><h3>위험자산 70% 룰</h3></div><span class="pension-insight-badge ${riskTone==='danger'?'danger':'safe'}">현재 ${risk.ratio.toFixed(1)}%</span></div><div class="pension-risk-compact-row"><div><span>위험자산</span><strong>${won(risk.riskEval)}</strong></div><div><span>안전자산</span><strong>${won(risk.safeEval)}</strong></div><div><span>기준 대비</span><strong class="${risk.gap>0?'negative':'positive'}">${risk.gap>0?'+':''}${risk.gap.toFixed(1)}%p</strong></div></div><div class="pension-risk-gauge compact" title="위험자산 ${won(risk.riskEval)} / 안전자산 ${won(risk.safeEval)} / 기준 대비 ${risk.gap>0?'+':''}${risk.gap.toFixed(1)}%p"><div class="pension-risk-fill ${riskTone==='danger'?'danger':'safe'}" style="width:${gaugeWidth.toFixed(1)}%"></div><div class="pension-risk-threshold" style="left:${risk.threshold}%"><span>${risk.threshold}%</span></div></div><div class="pension-risk-scale"><span>0%</span><span>기준 ${risk.threshold}%</span><span>100%</span></div><p class="small pension-risk-note">※ 채권형·현금성 자산 제외 기준</p></div></div>`;
}
