...
function renderSourceTables(x){
  const c=PORTFOLIO.constants,
    ...
    cash=Number(x.securitiesCash||0),
    holdingPlusCash=holdingCostPrincipal+cash,
    extraRow=extraContribution?`<tr><td>추가 외부투입</td><td class="num">${fmt(extraContribution)}</td></tr>`:'',
    excludedRow=excludedTransfer?`<tr><td>6~8월 별도수익 재투입</td><td class="num">${fmt(excludedTransfer)}</td></tr>`:'',
...
function drawPensionStacked(){
  ...
  labelDates(svg,cfg,data,3);
  addHover(svg,cfg,data,d=>{let html=`<div class="tt-date">${d['날짜']}</div>`;const total=series.reduce((a,s)=>a+Number(d[s]||0),0);series.forEach(s=>html+=row(s,won(d[s]||0),''));return html+'<div style="height:6px"></div>'+row('평가총액',won(total),'')});
}
