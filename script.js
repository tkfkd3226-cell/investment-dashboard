...
function mobileViewToggle(key){
  const mode=MOBILE_VIEW_MODES[key]||'card';
  const label=mode==='card'?'표로 보기':'카드로 보기';
...
function toggleMobileDataView(key){
  ...
  document.querySelectorAll(`[data-mobile-view-button="${key}"]`).forEach(btn=>btn.textContent=next==='card'?'표로 보기':'카드로 보기');
}
...
function renderPensionContributionList(target='cashSnapshot'){
  ...
  if(!items.length){
    const label=selectedTarget==='contribution'?'기업적립금':(selectedTarget==='etfTrade'?'ETF 매수 반영':'현금성자산 평가금액');
...
<div class="pension-contrib-tool modal-card-box">
  ...
  <p class="small">ETF 매수 반영은 퇴직연금 앱 보유현황에 실제 반영된 날 저장하세요. 체결일·상품·수량·체결금액만 입력하면 나머지는 자동 계산합니다.</p>
  <div class="contrib-field full contrib-target-field"><span class="contrib-field-label">등록 유형</span><input type="hidden" id="pensionContribTarget" value="cashSnapshot"><div class="contrib-target-tabs" role="tablist" aria-label="등록 유형 선택"><button type="button" class="contrib-target-option active" data-target="cashSnapshot" onclick="setPensionContributionTarget('cashSnapshot')">현금성자산 평가금액</button><button type="button" class="contrib-target-option" data-target="contribution" onclick="setPensionContributionTarget('contribution')">기업적립금</button><button type="button" class="contrib-target-option" data-target="etfTrade" onclick="setPensionContributionTarget('etfTrade')">ETF 매수 반영</button></div></div>
...
function renderHoldings(x){
  ...
  const cards=orderedHoldings.map(h=>mobileInfoCard(`<span class="holding-name-text">${h.name}</span>${securitySymbolSwatch(h.name)}`,[
    ...
    ['수수료반영 손익',won(h.feeAdjustedProfit ?? h.profit),cls(h.feeAdjustedProfit ?? h.profit)],
    ['손익률',pct(h.returnRate),cls(h.returnRate)]
  ],h.cssClass||'')).join('')+
  mobileInfoCard('보유종목 합계',[
    ['투자원금',won(holdCost)],['평가금액',won(holdEval)],['평가손익',won(holdProfit),cls(holdProfit)],['수수료반영 손익',won(holdFeeAdjusted),cls(holdFeeAdjusted)],['손익률',pct(holdReturn),cls(holdReturn)]
  ],'summary-card')+
  mobileInfoCard('증권계좌 현금',[
    ['투자원금',won(cash)],['평가금액',won(cash)],['평가손익',won(0)],['수수료반영 손익',won(0)],['손익률',pct(0)]
  ])+
  mobileInfoCard('총계(보유분+현금)',[
    ['투자원금',won(totalCostWithCash)],['평가금액',won(totalEvalWithCash)],['평가손익',won(holdProfit),cls(holdProfit)],['수수료반영 손익',won(holdFeeAdjusted),cls(holdFeeAdjusted)],['손익률',pct(totalReturnWithCash),cls(totalReturnWithCash)]
  ],'summary-card');
  return `<section id="securities-holdings" ${mobileViewAttrs('holdings')}><div class="section-title"><h2><span class="section-title-icon">📁</span>증권계좌 보유분</h2>${mobileViewToggle('holdings')}</div><div class="mobile-scroll table-view"><table class="hold-position-table"><thead><tr><th>종목명</th><th>수량</th><th>평단</th><th>투자원금</th><th>현재가</th><th>평가금액</th><th>평가손익</th><th>수수료반영 손익</th><th>손익률</th></tr></thead><tbody>${orderedHoldings.map(h=>`<tr class="hold-row ${h.cssClass||''}"><td><span class="holding-name-text">${h.name}</span>${securitySymbolSwatch(h.name)}</td><td class="num">${fmt(h.qty)}</td><td class="num">${fmt(h.avgPrice ?? (h.qty?h.cost/h.qty:0))}</td><td class="num">${fmt(h.cost)}</td><td class="num">${fmt(h.price)}</td><td class="num">${fmt(h.evalAmount)}</td><td class="num ${cls(h.profit)}">${fmt(h.profit)}</td><td class="num ${cls(h.feeAdjustedProfit ?? h.profit)}">${fmt(h.feeAdjustedProfit ?? h.profit)}</td><td class="num ${cls(h.returnRate)}">${pct(h.returnRate)}</td></tr>`).join('')}<tr class="summary-row"><td>보유종목 합계</td><td class="num">-</td><td class="num">-</td><td class="num">${fmt(holdCost)}</td><td class="num">-</td><td class="num">${fmt(holdEval)}</td><td class="num ${cls(holdProfit)}">${fmt(holdProfit)}</td><td class="num ${cls(holdFeeAdjusted)}">${fmt(holdFeeAdjusted)}</td><td class="num ${cls(holdReturn)}">${pct(holdReturn)}</td></tr><tr><td>증권계좌 현금</td><td class="num">-</td><td class="num">-</td><td class="num">${fmt(cash)}</td><td class="num">-</td><td class="num">${fmt(cash)}</td><td class="num">0</td><td class="num">0</td><td class="num">0.00%</td></tr><tr class="summary-row"><td>총계(보유분+현금)</td><td class="num">-</td><td class="num">-</td><td class="num">${fmt(totalCostWithCash)}</td><td class="num">-</td><td class="num">${fmt(totalEvalWithCash)}</td><td class="num ${cls(holdProfit)}">${fmt(holdProfit)}</td><td class="num ${cls(holdFeeAdjusted)}">${fmt(holdFeeAdjusted)}</td><td class="num ${cls(totalReturnWithCash)}">${pct(totalReturnWithCash)}</td></tr></tbody></table></div><div class="mobile-card-view">${cards}</div></section>`;
}
function renderPensionProductsBlock(x,pensionCashCost,pensionHeldCost,pensionHeldProfit,pensionHeldReturn){
  const cashProfit=x.pensionCash-pensionCashCost,
...
function renderCombined(x){
  const cards=mobileInfoCard('퇴직연금',[
    ...
    ['투입원금',won(x.totalPrincipal)],['투자 결과물',won(x.totalResult)],['누적손익',won(x.totalProfit),cls(x.totalProfit)],['투자대비 이익률',pct(x.returnRate),cls(x.returnRate)]
  ])+mobileInfoCard('합산',[
    ['투입원금',won(x.combinedPrincipal)],['투자 결과물',won(x.combinedResult)],['누적손익',won(x.combinedProfit),cls(x.combinedProfit)],['투자대비 이익률',pct(x.combinedReturn),cls(x.combinedReturn)]
  ],'summary-card');
  return `<section id="summary-section" ${mobileViewAttrs('combined')}><div class="section-title"><h2><span class="section-title-icon">🏠</span>연금+계좌 성과</h2>${mobileViewToggle('combined')}</div><div class="mobile-scroll table-view"><table class="combined-performance-table"><thead><tr><th>구분</th><th>투입원금</th><th>투자 결과물</th><th>누적손익</th><th>투자대비 이익률</th></tr></thead><tbody><tr><td><strong>퇴직연금</strong></td><td class="num">${fmt(x.pensionPrincipal)}</td><td class="num">${fmt(x.pensionEval)}</td><td class="num ${cls(x.pensionProfit)}">${fmt(x.pensionProfit)}</td><td class="num ${cls(x.pensionReturn)}">${pct(x.pensionReturn)}</td></tr><tr><td><strong>증권계좌</strong></td><td class="num">${fmt(x.totalPrincipal)}</td><td class="num">${fmt(x.totalResult)}</td><td class="num ${cls(x.totalProfit)}">${fmt(x.totalProfit)}</td><td class="num ${cls(x.returnRate)}">${pct(x.returnRate)}</td></tr><tr class="summary-row"><td>합산</td><td class="num">${fmt(x.combinedPrincipal)}</td><td class="num">${fmt(x.combinedResult)}</td><td class="num ${cls(x.combinedProfit)}">${fmt(x.combinedProfit)}</td><td class="num ${cls(x.combinedReturn)}">${pct(x.combinedReturn)}</td></tr></tbody></table></div><div class="mobile-card-view">${cards}</div></section>`;
}
...
function pensionProductCard(h,total){const contrib=total?h.profit/total*100:0,cr=h.prevEval?h.dayChange/h.prevEval*100:null;return `<div class="mini-card symbol-card"><div class="m-label">${h.name}${pensionProductSwatch(h.name)}</div><div class="m-value ${cls(h.profit)}">${won(h.profit)}</div><div class="symbol-metrics"><div class="symbol-metric"><span class="symbol-metric-label">기여도</span><span class="symbol-metric-value ${cls(contrib)}">${pct(contrib)}</span></div><div class="symbol-metric"><span class="symbol-metric-label">전일대비 변동액</span><span class="symbol-metric-value ${cls(h.dayChange)}">${h.dayChange==null?'-':signed(h.dayChange,'원')}</span></div><div class="symbol-metric"><span class="symbol-metric-label">전일대비 변동률</span><span class="symbol-metric-value ${cr==null?'':cls(cr)}">${cr==null?'-':((cr>0?'+':'')+pct(cr))}</span></div></div></div>`}
...
function drawLineChart(){
  ...
  const series=['SK하이닉스','삼성전자','현대차','KODEX 200'],colors=SECURITY_SYMBOL_COLORS,values=data.flatMap(d=>series.map(s=>d[s]||0));
...
function buildPensionContributionItem(){
  ...
  if(target==='etfTrade'){
    ...
    const expected=pensionEtfTradeExpected(draft);
    if(!expected) throw new Error('ETF 매수 예상값을 계산하지 못했습니다.');
